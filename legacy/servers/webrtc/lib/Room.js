...
class Room extends EventEmitter
{
	 ...
	 */
	static async create({ mediasoupWorkers, roomId, channelId, peers })
	{
	...
	})
	{
		...
		// save to redis
		redisClient.set(`meeting.${this._channelId}`, serialize(this._redisEntry));
	...
	async close()
	{
		...
		for (const peer in this._peers)
		{
			if (!this._peers[peer].closed)
				this._peers[peer].close("meeting-terminated");
		}
		...
		if(this._ownerToken) {
			...
			if(this._channelId) {
				await this.deleteRedisEntry();
				await axios.get(`${config.apiEndpoint}/api/v1/call.status`, { params: { call_hash: this._roomId, channel_id: this._channelId }, validateStatus: false });
			}

			await axios.post(`${config.apiEndpoint}/api/v1/call.end`, qs.stringify({ call_hash: this._roomId, rtm_token: this._ownerToken }), { validateStatus: false, headers: {
	...
	deleteRedisEntry() {
		return new Promise((resolve, reject) => {
			redisClient.del(`meeting.${this._channelId}`, (err, response) => {
				if (response == 1) {
					resolve(response);
				} else{
					reject(err);
						...
						const { data } = await axios.get(
							config.turnAPIURI,
							{
								...
								params  : {
									...
									'ip'      : peer.socket._socket.remoteAddress
								}
							});
	...
	async _updateRedisCache() {

		if(!this._channelId || this._channelId == 0) return;

		this._redisEntry = {
			...this._redisEntry,
			last_updated_at: moment().unix(),
			peers: this.getJoinedPeers().map((joinedPeer) => {
				return {
					displayName: joinedPeer._displayName,
					id: joinedPeer._id
				};
			})
		}
		...
		axios.get(`${config.apiEndpoint}/api/v1/call.status`, { params: { call_hash: this._roomId, channel_id: this._channelId }, validateStatus: false })
	...
	async _handleSocketRequest(peer, request, cb)
	{
		...
		switch (request.method)
		{
			...
			case 'chatMessage':
			{
				...
				this._chatHistory.push(parsedMessage);

				// Spread to others
				this._notification(peer.socket, 'chatMessage', parsedMessage, true);
			...
			case 'moderator:closeMeeting':
			{
				...
				this._notification(peer.socket, 'moderator:closed', null,	true);
			...
			case 'moderator:goPublic':
			{
				...
				if(peer._socket.handshake.session && peer._socket.handshake.session.rtmToken) {

					if(this._channelId) {
						redisClient.del(`meeting.${this._channelId}`);
					}

					const remoteRequest = await axios.post(`${config.apiEndpoint}/api/v1/call.update`, qs.stringify({ call_hash: this._roomId, is_public: 1, allowed_users: "", rtm_token: peer._socket.handshake.session.rtmToken }), { validateStatus: false, headers: {
						'Content-Type': 'application/x-www-form-urlencoded'
					}})

					const { data: remoteResponse } = remoteRequest;

					this._notification(peer.socket, 'moderator:goPublic', null,	true, true);
	...
	{
		return new Promise((resolve, reject) => {
			socket.send(JSON.stringify({ type: 'request', method, data }));
			this._requests[data.id] = (err, response) => {
				if(err) {
					reject(err);
				} else {
					resolve(response);
	...
	_notification(socket, method, data = {}, broadcast = false, includeSender = false)
	{
		if (broadcast)
		{
			...
			for (const peer in this._peers)
			{
				console.log(peer, socket.handshake.session.peerId)
				if (this._peers[peer] && this._peers[peer]._socket && peer != socket.handshake.session.peerId) {
					this._peers[peer]._socket.send(JSON.stringify({ type: 'notification', method, data }));
				}
			}

			if (includeSender)
				socket.send(JSON.stringify({ type: 'notification', method, data }));
		}
		else
		{
			socket.send(JSON.stringify({ type: 'notification', method, data }));
