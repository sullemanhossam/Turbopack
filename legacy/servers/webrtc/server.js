...
process.env.DEBUG = 'edumeet-server:*';
...
console.log('- config.mediasoup.worker.rtcMinPort:', config.mediasoup.worker.rtcMinPort);
console.log('- config.mediasoup.worker.rtcMaxPort:', config.mediasoup.worker.rtcMaxPort);
...
async function runWebSocketServer()
{
	...
	try
	{
		...
		// send session to ws
		mainListener.on('upgrade', function(request, socket, head)
		{
			session(request, {}, () =>
			{
				...
				try
				{
					...
					switch (msg.type)
					{
						case 'auth':
							...
							const { peerId, roomId, rtmToken } = msg;

							// set session
							request.session.peerId = peerId;
							request.session.roomId = roomId;
							request.session.rtmToken = rtmToken;
							...
							queue.push(async () =>
							{
								...
								// authenticate user
								const remoteRequest = await axios.get(`${config.apiEndpoint}/api/v1/call.join`, { params: { call_hash: roomId, rtm_token: rtmToken }, validateStatus: false })
								const { data: remoteResponse } = remoteRequest;

								if(!remoteResponse.meta.ok) {
									logger.warn(
										'disconnecting due to invalid call.join [roomId:"%s", peerId:"%s"]', request.session.roomId, request.session.peerId);
									ws.close(1011, 'auth-unauthorized');
									return;
								}

								const { user, call } = remoteResponse.info;

								const { token } = request.session;

								const room = await getOrCreateRoom({ roomId, channelId: (call.channel_id) ? parseInt(call.channel_id) : null });
								...
								if(peer) {
									peer.close('auth-duplicated-connection');
									returning = true;
								}
								...
								{ // Don't allow hijacking sessions
									...
									logger.warn(
										'disconnecting due to session hijacking [roomId:"%s", peerId:"%s"]', request.session.roomId, request.session.peerId);
								...
								*/
								...
								if(call.is_public === '0' && user && user.display_name) {

									peer.displayName = user.display_name;
									peer.picture = user.has_avatar;
								...
								} else if(peerId == call.user_id){
									peer.addRole(userRoles.ADMIN);
									peer.addRole(userRoles.MODERATOR);
									peer.addRole(userRoles.AUTHENTICATED);
								}

								if(peerId == call.user_id) {
									room.setOwnerToken(rtmToken);
								}
						...
						case 'request':
							...
							room._handleSocketRequest(peer, msg, (err, response) => {
								peer.socket.send(JSON.stringify({ type: 'request', id: msg.id, err, response }));
							})

							break;
						case 'callback':

							const currentPeer = peers.get(session.peerId);
							const currentRoom = rooms.get(currentPeer._roomId);

							if (currentRoom._requests && currentRoom._requests[msg.id]) {
								currentRoom._requests[msg.id](msg.data.err, msg.data.response);
							}
					...
					console.log("INVALID MSG", e, message);
				}
			});
 ...
 */
async function getOrCreateRoom({ roomId, channelId })
{
	...
	if (!room)
	{
		...
		room = await Room.create({ mediasoupWorkers, roomId, channelId, peers });
