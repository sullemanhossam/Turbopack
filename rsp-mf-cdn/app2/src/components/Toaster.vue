    ...
    <transition-group name="list" tag="div">
      ...
      >
        <div class="toast-item-message" v-if="toast.isUpdate">
          {{ typeof toast.content === "string" ? toast.content : $t(toast.content.message, toast.content.meta) }}
    ...
    if (window.ipcRenderer) {
      window.ipcRenderer.on("download-available", () => {
        this.$store.dispatch("core/appUpdatesCompleted", {
          close: true,
          isUpdate: true,
          content: "A new update for AirSend has been downloaded. Please restart to install the updates",
          contentType: "text",
