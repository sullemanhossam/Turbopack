<template>
  <div>
    <form v-if="step === 0" id="login" novalidate="true" @submit="onLogin">
      ...
      <div v-if="this.env.VUE_APP_GOOGLE_ID && this.env.VUE_APP_GOOGLE_ID !== ''" class="form-separator">
      ...
      <div class="row">
        <div class="col-6">
          <div class="form-group form-check">
            <input id="remember_me" v-model="login.remember_me" type="checkbox" class="form-check-input" />
            <label class="form-check-label" for="remember_me">{{ $t("auth.remember") }}</label>
          </div>
        </div>

        <div class="col-6">
          <div class="form-group text-right">
            <a href="#" class="h6" @click="browse($event, 'recover')">{{ $t("auth.forgot-password") }}</a>
          </div>
        </div>
      </div>

      <div class="text-center d-block">
        <button style="min-width: 50%" type="submit" class="btn btn-half btn-primary btn-rounded btn-extended mx-auto d-block mt-4">
          {{ $t("auth.login") }}
        </button>
        <button
          v-if="!this.user.id"
    ...
    <form v-if="step === 1" novalidate="true" @submit="onVerify">
      <div class="form-group">
        <label class="text-center text-info" for="email">{{ $t("auth.login-verification") }}</label>
      </div>
...
export default {
  ...
  mounted() {
    ...
    if (window.isElectron) {
      ...
      ipcRenderer.on("Signin-Token", async (event, arg) => {
        const response = await this.$store.dispatch("core/loginWithEmailAndToken", arg);
  ...
  methods: {
    ...
    async refreshCode(e) {
      ...
      const response = await this.$store.dispatch("core/refreshVerificationCode", { email: this.login.email });
