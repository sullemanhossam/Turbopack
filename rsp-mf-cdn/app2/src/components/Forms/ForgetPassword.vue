  ...
  <div>
    ...
    <div v-if="isDone">
      <div class="form-group">
        <label class="text-center text-success" for="email">{{ $t("auth.recover-confirmation") }}</label>
      </div>

      <div class="text-center d-block">
        <button type="button" class="btn btn-primary btn-ghost btn-rounded btn-extended mx-auto p-relative mt-4" @click="browse($event, 'login')">
    ...
    <form v-else novalidate="true" @submit="onSubmit">
      ...
      <div class="form-group">
        <label class="text-center" for="email">{{ $t("auth.recover-instructions") }}</label>
      </div>
      ...
      <div class="form-group" :class="{ [`is-invalid`]: errors['opt_email'] }">
        ...
        <input
          ...
          autofocus
        />
        <small v-if="errors['opt_email']" class="form-text text-danger">{{ $t(errors["opt_email"].message, errors["opt_email"].meta) }}</small>
      </div>
