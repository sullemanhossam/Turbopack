<template>
  <div>
    <form v-if="step === 0" id="registration" novalidate="true" @submit="onSignup">
      <Loader :loading="this.$store.state.loading['user.create']" full />

      <div v-if="isElectron" class="mb-3 oauth-signin-button" @click="initiateGoogleLogin">
        <GoogleLogo /> <span>{{ $t("auth.sign-with-google") }}</span>
      </div>
      <div
        v-if="this.env.VUE_APP_GOOGLE_ID && this.env.VUE_APP_GOOGLE_ID !== '' && !isElectron"
        id="google-signin-button"
        class="mb-3 google-signin-button"
      ></div>

      <div
        v-if="isElectron && this.env.VUE_APP_APPLE_ID && this.env.VUE_APP_APPLE_ID !== ''"
      ...
      <div class="form-group" :class="{ [`is-invalid`]: errors['name'] }">
        ...
        <input
          ...
          autofocus
        />
        <small v-if="errors['name']" class="form-text text-danger">{{ $t(errors["name"].message, errors["name"].meta) }}</small>
      </div>
      ...
      <div class="form-group" :class="{ [`is-invalid`]: errors['password'] }">
        ...
        <input id="password" v-model="signup.password" type="password" class="form-control form-control--underline" placeholder="******" />
        <small v-if="errors['password']" class="form-text text-danger">{{ $t(errors["password"].message, errors["password"].meta) }}</small>
      </div>

      <div class="form-group" :class="{ [`is-invalid`]: errors['recaptcha'] }">
        <div id="recaptchav2"></div>
        <small v-if="errors['recaptcha']" class="form-text text-danger">{{ $t(errors["recaptcha"]) }}</small>
      </div>
    ...
    <form v-if="step === 1" id="registration" novalidate="true" @submit="onVerify">
      <div class="form-group">
        <label class="text-center text-info" for="email">{{ $t("auth.signup-confirmation") }}</label>
      </div>
      ...
      <div class="form-group" :class="{ [`is-invalid`]: errors['verify_code'] }">
        ...
        <input
          ...
          autofocus
        />
        <small v-if="errors['verify_code']" class="form-text text-danger">{{ $t(errors["verify_code"].message, errors["verify_code"].meta) }}</small>
      </div>

      <div class="form-group text-left">
        <a href="#" class="h6" :class="{ disabled: refreshBlock > 0 }" @click="refreshCode">{{
          refreshBlock > 0 ? $t("auth.verification-code-resend-blocked", [refreshBlock]) : $t("auth.verification-code-resend")
        }}</a>
      </div>
...
export default {
  ...
  watch: {
    reCaptchaV2Required(newValue, oldValue) {
      if (newValue !== oldValue && newValue) {
        const recaptchaV2Key = _.get(this.$store.state, "core.handshakeSettings.recaptchaV2SiteKey", "");
  ...
  mounted() {
    ...
    if (this.env.VUE_APP_APPLE_ID && this.env.VUE_APP_APPLE_ID !== "") {
      if (window.AppleID) {
        this.appleRedirectUri = _.get(this.env, "VUE_APP_APPLE_REDIRECT_URL", `${window.location.protocol}//${window.location.host}/oauth/apple`);
    ...
    if (this.$store.state.core.recaptchaV2Required) {
      grecaptcha.render("recaptchav2", {
        sitekey: _.get(this.$store.state.core, "handshakeSettings.recaptchaV2SiteKey", ""),
      });
    }
  },
  methods: {
    ...
    async refreshCode(e) {
      ...
      const response = await this.$store.dispatch("core/refreshVerificationCode", { email: this.signup.email });
