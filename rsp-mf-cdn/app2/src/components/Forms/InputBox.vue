<template>
  <div class="form-group col-sm" :class="{ [`is-invalid`]: errors[id] }">
    <label v-if="label" :class="{ [`is-optional`]: optional }" :for="id"
      >{{ label }}{{ required ? "*" : "" }}
      <Icon v-if="hint" v-tooltip="{ content: hint }" class="hint" name="info-circle" family="fas" />
    ...
    >
      ...
      >
        <Icon
          ...
          :class="`input-group-text ${prepend.iconBold ? 'font-weight-bold' : ''} ${prepend.action ? 'cursor-pointer' : ''} border-0`"
          family="fal"
          :name="prepend.icon"
          :animation="prepend.animation"
        />
      </div>
      <div v-if="$slots['input-prepend']" class="input-group-prepend" @click="addFocus">
      ...
      <input
        ...
        @copy="onCopy"
      />
      <div v-if="type === 'file' && currentSelection.length > 0" class="files-selection">
        {{ currentSelection[0].name }}
      </div>
      <div v-if="$slots['input-append']" class="input-group-append" @click="addFocus">
      ...
      <div
        ...
        @keydown.enter.prevent="append.action ? append.action() : defaultAction()"
        @keydown.space.prevent="append.action ? append.action() : defaultAction()"
      >
        <Icon
          :ref="append.ref ? append.ref : `${append.icon}-icon-ref`"
          :class="`input-group-text  ${append.iconBold ? 'font-weight-bold' : ''}  ${append.action ? 'cursor-pointer' : ''} border-0`"
          :title="append.title"
          family="fal"
          :name="append.icon"
        />
      ...
      <div v-if="type === 'file'" class="input-group-append">
        <button v-if="currentSelection.length > 0" class="btn btn-cancel" type="button" @click="onCancelUpload" @focus="onFocus">
          <Icon family="fas" name="times" />
        </button>
        <button v-else class="btn btn-upload" type="button" @click="onFocusUpload" @focus="onFocus">
          {{ $t("Upload") }}
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    updateValue: function (e) {
      if (this.type === "file") {
        ...
        for (let i = 0; i < items.length; i++) {
          const entry = items[i] instanceof DataTransferItem ? items[i].webkitGetAsEntry() : items[i];
    ...
    onFocusUpload(e) {
      ...
      if (e.target.className.indexOf("btn-upload") > -1 && e.target.className.indexOf("btn-cancel") === -1) {
