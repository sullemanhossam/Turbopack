<template>
  <fragment>
    ...
    >
      <ul v-if="isCommand && message !== '' && commands.length > 0" class="chatbar-commands">
      ...
      <fragment v-if="!channel || user.role.can('channel.message')">
        ...
        <button v-if="!isEditing" class="btn btn-icon" @click="() => this.$refs.file.click()">
          <input ref="file" type="file" hidden multiple @change="onChooseFile" />
        ...
        <div class="chatbar">
          ...
          <div v-if="files && files.length > 0" class="chatbar-extension chatbar-extension--attachments">
            <button class="btn btn-icon" @click="cancelFiles">
              <Icon family="far" name="times" class="close-btn" />
            </button>

            <ul class="attachment-list">
              <li v-for="(file, index) in files.slice(0, files.length > 3 ? 2 : 3)" :key="index">
              ...
              <li v-if="files.length > 3">
                <Popover :boundaries-element="$refs.container" popover-base-class="popover tooltip popover-attachments">
                  <div class="single-attachment">
                    <FileIcon name="more" />
                    <div>
                      <p>
          ...
          <div class="chatbar-holder">
            ...
            >
              <div class="popover-body">
                <ul class="popover-mention" v-if="filteredUsers.length > 0 && isMentioningUser">
                  <li v-for="(user, index) in filteredUsers" :class="{ 'is-selected': navigatedUserIndex === index }" :key="user.id">
                ...
                <ul v-else-if="isMentioningResource && this.channel.action_count === 0" class="popover-mention">
                  <li>
                    <span class="no-content">{{ $t("actions.no-actions-for-the-channel") }}</span>
                  </li>
                  <li>
                ...
                <ul class="popover-mention" v-else-if="filteredMentionableResource.length > 0 && isMentioningResource">
                  <li
        ...
        <div class="btn btn-icon btn-send" @click="submitMessage">
          ...
          <Icon v-else-if="!isEditing && !shouldSendEmail" family="fas" name="paper-plane" />
          <Icon v-else-if="!isEditing && shouldSendEmail" family="fas" name="envelope" />
          <Icon v-if="isEditing" family="fal" name="times" />
        </div>

        <Popover v-if="this.channel && !channel.public_url" popover-base-class="popover tooltip popover-attachments">
          <div class="btn btn-icon">
            <Icon family="fal" name="ellipsis-h" />
          </div>
          <template slot="popover">
            <div class="dropdown-items">
              <button v-close-popover class="dropdown-item" :class="{ active: !shouldSendEmail }" type="button" @click="onSelectSendType(false)">
                <Icon family="fal" name="paper-plane" />
                {{ $t("channels.chat-message-only") }}
              </button>
              <button v-close-popover class="dropdown-item" :class="{ active: shouldSendEmail }" type="button" @click="onSelectSendType(true)">
    ...
    <div v-else-if="IS_READONLY" class="chat-fragment-footer">
      ...
      <div
        v-else
        class="chat-fragment-footer--read-only"
        @click="onClickLink($event, user.account_status === 50 ? 'finalize' : 'auth')"
        v-html="$t(`channels.chat-read-only-${user.account_status === 50 ? 'finalize' : 'auth'}`)"
      ></div>
    </div>
  </fragment>
</template>
...
import { retrieveFiles, bytesToSize, parseMentions, parseTime, htmlToMessage } from "airsend/utils";
...
export default {
  ...
  computed: {
      ...
      return {
        ...
        menuItemTemplate: (item) => {
          ...
          if (item.original.is_all) {
            avatar = '<div class="avatar"><i class="inline-icon fa fa-users"></i></div>';
    ...
    contextActions() {
      return this.$store.getters["actions/getActionsByChannel"](this.$route.params.id);
    },
    commands() {
      return this.channel
        ? _.filter(this.channel.commands, (item) => {
            return (
              ...
              (this.user.role.level >= item.level || this.user.id === this.channel.created_by)
            );
          })
        : [];
    },
    ...
    placeholderText() {
      return this.isMobile || this.tipVal === 0 ? this.$t("channels.chat-say-something") : this.$t(`channels.chat-say-something-${this.tipVal}`);
    },
  },
  watch: {
    $route(to, from) {
  ...
  methods: {
    load() {
      ...
      // send email param
      this.shouldSendEmail = store.get(`send_type_${id}`) ? store.get(`send_type_${id}`) : false;
    ...
    async submitMessage(e) {
      ...
      if (
        ...
        (this.channel.chat.buffer && this.channel.chat.buffer.newer && this.channel.chat.buffer.newer.length > 0)
      ) {
    ...
    onChangeMessage(e) {
      ...
      const message = htmlToMessage(e ? e.target.innerHTML : this.$refs.editor.innerHTML);
    ...
    onKeyUp(e) {
      ...
      if (key === "Backspace") {
        ...
        if (lastIndexOfHashtag > lastIndexOfSpace && (lastIndexOfHashtag === 0 || this.message[lastIndexOfHashtag - 1] === " ")) {
          // this is a test message with #mention...
          this.shouldStartMentionOfType = "resource";
          this.startOffset = lastIndexOfHashtag + 1;
        }
        if (lastIndexOfAt > lastIndexOfSpace && (lastIndexOfAt === 0 || this.message[lastIndexOfAt - 1] === " ")) {
      ...
      if (this.shouldStartUserMention || this.shouldStartResourceMention) {
        ...
        this.leftPopoverBound = bounds.left - editorBounds.left - popoverBounds.width / 2;
      ...
      if (this.isMentioningUser || this.isMentioningResource) {
        ...
        // Enter
        if (keyCode === 13) {
          if (this.isMentioningUser && this.filteredUsers[this.navigatedUserIndex]) {
            this.insertMention(this.filteredUsers[this.navigatedUserIndex]);
          } else if (this.isMentioningResource && !this.isFetchingMentionableResource && this.filteredMentionableResource[this.navigatedUserIndex]) {
            this.insertMention(this.filteredMentionableResource[this.navigatedUserIndex]);
          }
          return;
        }
    ...
    onKeyDown(e) {
      ...
      if (this.isMentioning) {
        if (key === "Enter") {
          if (
            (this.isMentioningUser && this.filteredUsers[this.navigatedUserIndex]) ||
            (this.isMentioningResource && this.filteredMentionableResource[this.navigatedUserIndex])
          ) {
        ...
        if (key === "ArrowDown") {
          ...
          if (this.isMentioningUser) {
            if (this.navigatedUserIndex < this.filteredUsers.length - 1) this.navigatedUserIndex++;
          } else if (this.isMentioningResource) {
            if (this.navigatedUserIndex < this.filteredMentionableResource.length - 1) this.navigatedUserIndex++;
          }
          return;
        }
      ...
      if (key == "@") {
        if (this.message.length === 0 || this.getCharacterPrecedingCaret() === " ") {
    ...
    insertMention(resource) {
      ...
      } else if (this.isMentioningResource && resource.resource_type === "action") {
        this.insertNode(
    ...
    getLastMessage() {
      if (!this.channel || !this.channel.chat || !this.channel.chat.history) return;
    ...
    onPaste(e) {
      ...
      if (items) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (file) {
            files.push(new File([file], `clipboard-${moment().format("YYYY-MM-DD-hhmmss")}.png`));
          }
        }
