<template>
  <ul class="popover-mention" :class="{ hidden: query[0] == ' ' || query[0] == '#' }">
    <li :class="{ 'is-selected': index === selectedIndex }" v-for="(item, index) in filteredActions" :key="index">
    ...
    <li v-if="filteredActions.length === 0">
      <a @click="onClickAddAction">
        ...
        {{ $t(query !== "" ? "actions.add-action-filled" : "actions.add-action", { actionName: query }) }}
      </a>
    </li>

    <div class="popover-mention-loading" v-if="isLoading && filteredActions.length > 0">
      {{ $t("general.loading") }}
    </div>
  </ul>
</template>
...
export default {
  ...
  computed: {
    actions() {
      const inMemoryActions = this.$store.getters["actions/getActionsByChannel"](this.$parent.channel.id);
  ...
  methods: {
    ...
    upHandler() {
      this.selectedIndex = (this.selectedIndex + this.filteredActions.length - 1) % this.filteredActions.length;
    },
    ...
    async fetchMentionableResource(query) {
      if (this.filteredActions.length === 0 && this.lastQuery.length > query.length) {
      ...
      if (!query || query === "") {
        ...
        this.isLoading = true;
        const responseActions = await this.$store.dispatch("actions/searchMentionableActions", { channel_id: this.$parent.channel.id, query });
