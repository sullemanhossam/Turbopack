    ...
    >
      ...
      <fragment v-if="!channel || user.role.can('channel.message')">
        ...
        <div class="btn btn-icon btn-send" @click="submitMessage">
          <Icon v-if="!isEditing && !shouldSendEmail" family="fas" name="paper-plane" />
...
import { retrieveFiles, bytesToSize, parseMentions, parseTime, htmlToMessage, wrapText } from "airsend/utils";
    ...
    placeholderText() {
      return this.isMobile || this.tipVal === 0
        ...
        : this.$t(`channels.chat-say-something-${this.tipVal}`, {
            metaKey: /Mac|iPod|iPhone|iPad/.test(navigator.platform) ? "Cmd" : "Ctrl",
          });
    },
  },
  watch: {
    ...
    this.editor = new Editor({
          ...
          addKeyboardShortcuts: () => {
            return {
              Enter: ({ editor }) => {
                ...
                if (!editor.isEmpty || (parentContext.files && parentContext.files.length > 0)) {
      ...
      onUpdate() {
        ...
        if (message !== "" && parentContext.channel) {
          parentContext.$store.dispatch("channels/sendTypingEvent", parentContext.channel.id);
        }
  ...
  methods: {
    load() {
      ...
      if (this.editor && this.editor.extensionManager) {
        for (var i = 0; i < this.editor.extensionManager.extensions.length; i++) {
          if (this.editor.extensionManager.extensions[i].options && this.editor.extensionManager.extensions[i].options.placeholder) {
            this.editor.extensionManager.extensions[i].options.placeholder = this.placeholderText;
          }
        }
      }
