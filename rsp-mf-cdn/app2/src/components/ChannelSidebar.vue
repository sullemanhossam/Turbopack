<template>
  <aside class="sidebar-fragment" :class="{ [`sidebar-fragment--minified`]: isMinified }">
    <button
      v-tooltip="{
        delay: 1000,
        offset: -5,
    ...
    >
      <Icon family="far" :name="isMinified ? 'arrow-to-left' : 'arrow-to-right'" />
    </button>
    ...
    <div class="sidebar-fragment-content">
      <ul class="nav nav-pills nav-fill nav-sidebar">
        <li class="nav-item" :set="(active = currentSection === 'actions')">
          <router-link
            ...
            :to="`/channel/${channel && channel.id}/actions${$route.query ? `?${qs({ hash: $route.query.hash })}` : ''}`"
            class="nav-link"
            :class="{ active }"
            ><Icon family="fal" name="bolt" /><span>{{ $t("general.actions") }}</span>
          </router-link>
        </li>
        <li class="nav-item" :set="(active = currentSection === 'files')">
          <router-link
            @click.native="onClickRoute"
            :to="`/channel/${channel && channel.id}/files${$route.query ? `?${qs({ hash: $route.query.hash })}` : ''}`"
            class="nav-link"
            :class="{ active }"
            ><Icon family="fal" name="file-alt" /><span>{{ $t("general.files-and-links") }}</span>
            <Progress v-if="isMinified && currentTransaction" :value="currentTransaction && currentTransaction.progress" />
          </router-link>
        </li>
        <li class="nav-item" :set="(active = currentSection === 'wiki')">
          <router-link
            @click.native="onClickRoute"
            :to="`/channel/${channel && channel.id}/wiki${$route.query ? `?${qs({ hash: $route.query.hash })}` : ''}`"
            class="nav-link"
            :class="{ active }"
            ><Icon family="fal" name="info-circle" /><span>{{ $t("general.wiki") }}</span>
          </router-link>
        </li>
      </ul>
      ...
      <Wiki ref="wiki" :active="currentSection === 'wiki'" :channel="channel" v-if="currentSection === 'wiki'" />
    </div>
  </aside>
</template>
...
export default {
  ...
  computed: {
    ...
    IS_SUPERUSER() {
      return this.channel && (this.channel.user_role === 100 || this.channel.user_role === 50);
    },
  },
  ...
  methods: {
    ...
    onWikiPreview(params) {
      ...
      const [pathType, channelDir, ...finalPath] = params.file.replace(/^\/|\/$/g, "").split("/");
      this.$router.push(`/channel/${this.channel.id}/wiki/${finalPath.join("/")}`).catch(() => {});
    },
    toggleMinification() {
