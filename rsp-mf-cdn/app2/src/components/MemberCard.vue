    ...
    >
      ...
      <template slot="popover">
        ...
        <div class="member-card-description">
          ...
          <div class="last-seen" v-if="author.last_active_on && !author.online_status">
            {{
          ...
          <button @click="createOneToOne" class="btn btn-link btn-sm mt-2" v-if="author.id !== user.id && !IS_READONLY">
...
export default {
  ...
  computed: {
    ...
    isGuest() {
      const teamMembers = this.$store.getters["teams/getMembers"][this.team_id] || [];
  ...
  methods: {
    ...
    async createOneToOne() {
      const response = await this.$store.dispatch("channels/createOneOnOne", this.author.id);
    },
  },
  components: {
    Avatar,
