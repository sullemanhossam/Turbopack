  ...
  <div v-if="users && users.length > 0" class="users-chips">
    <Popover>
      <div class="users-icons avatar-list">
        <div class="avatar-wrapper" v-for="user in users.length === 3 ? users.slice(0, 3) : users.slice(0, 2)" :key="`${user.id}-user-chip`">
          <Avatar
      ...
      <template slot="popover">
        <perfect-scrollbar class="popover-people">
          <li v-for="user in users" :key="user.id" class="popover-people-single">
            <Avatar
              :name="user.display_name"
              :user-id="parseInt(user.id)"
              :has-avatar="user.has_avatar"
...
export default {
  ...
  computed: {
    ...
    teamMembers() {
      return this.$store.getters["teams/getMembers"][this.team && this.team.id] || [];
    },
  },
  methods: {
    isGuest(user_id) {
