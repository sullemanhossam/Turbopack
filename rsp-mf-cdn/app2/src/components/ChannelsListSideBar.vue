    ...
    >
      ...
      >
        ...
        <div class="list-group-item-content">
          <h5>
            {{ !channel.one_one ? channel.channel_name : channel.counterpart ? channel.counterpart.display_name : "Direct Conversation" }}
          </h5>

          <UnreadBadge :unread_count="channel.unread_count" :muted="channel.muted" />
          ...
          <div v-if="channel.latest_message && channel.latest_message.content && !isCompactView" class="last-message">
            <span v-if="channel.latest_message.user_id !== 91000001"> {{ channel.last_message_author }}: </span>
            {{ channel.last_message_content }}
          </div>

          <div class="floating-controlls">
            <div class="more-options" @mouseover="hover = channel.id" @click.prevent.stop>
              <button
                v-tooltip="{
                  delay: 1000,
                  offset: -5,
              ...
              <Popover v-else>
                ...
                <template slot="popover">
                  <div class="dropdown-items">
                    ...
                    <button v-if="channel.muted" v-close-popover class="dropdown-item" type="button" @click.prevent.stop="toggleChannelMute(channel)">
                    ...
                    <button v-if="!channel.is_favorite" v-close-popover class="dropdown-item" type="button" @click.prevent="addToFavorite(channel)">
                      <Icon family="fal" name="thumbtack" />
                      {{ $t("channels.add-favorite") }}
                    </button>

                    <button v-else v-close-popover class="dropdown-item" type="button" @click.prevent="removeFromFavorite(channel)">
                      <Icon family="fas" name="thumbtack" />
                    ...
                    <button v-close-popover class="dropdown-item" type="button" @click.prevent="$emit('moveToGroup', channel)">
                      <Icon family="fal" name="arrow-alt-to-right" />
                      {{ $t("channels.groups.move-to-group") }}
                    </button>

                    <Popover v-if="channel.channel_status === 1 && ChannelRoles[channel.user_role].can('channel.manage')">
                    ...
                    <Popover
                      ...
                      :offset="-12"
                    >
                      <button v-close-popover class="dropdown-item d-flex align-items-center" type="button">
                        <Icon family="fal" name="ban" />
                        Block
                        {{ channel && channel.counterpart && channel.counterpart.display_name }}
                        <Icon family="fal" name="chevron-right" class="ml-3 mr-0" />
                      </button>
                      <template slot="popover">
                        <div class="dropdown-items team-filter-switcher">
                          <button v-close-popover class="dropdown-item" type="button" @click="() => blockChannel(channel)">
                            <span>Only block</span>
                          </button>

                          <button v-close-popover class="dropdown-item" type="button" @click="() => reportChannel(channel)">
                    ...
                    <Popover v-if="user.id !== channel.created_by && !channel.one_one">
                      <button class="dropdown-item" type="button">
                    ...
                    <Popover v-if="ChannelRoles[channel.user_role].can('channel.manage') && user.id === channel.created_by">
                      <button class="dropdown-item" type="button">
            ...
            <div class="join-meeting" v-if="channel.meeting && channel.id != currentMeeting.channelId">
              <button
                v-tooltip="{
                  delay: 1000,
                  offset: -5,
            ...
            <div class="join-meeting" v-if="channel.ongoing_call && channel.id !== currentMeeting.channelId">
              <button
                @click="
  ...
  computed: {
    ...
    channelList() {
      if (!this.showArchived) {
        return _.filter(this.channels, (o, i) => !o.is_autoarchived || o.unread_count != 0 || i < 5);
      } else {
  ...
  watch: {
    unreadChannelsList: {
      ...
      handler(unreadChannelsList, previusUnreadChannelsList) {
        ...
        let noLongerInUnreadList = Object.keys(previusUnreadChannelsList).reduce((diff, key) => {
          if (unreadChannelsList[key] && previusUnreadChannelsList[key]) return diff; //channel still has unread messages
