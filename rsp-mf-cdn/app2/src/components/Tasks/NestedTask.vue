      ...
      <perfect-scrollbar>
        <div class="task-table-row task-table-row-title" v-if="root">
          ...
          <span class="task-table-col-title task-title-col">
            Name
            <Icon family="fas" name="caret-down" v-if="fullActionFilters.sort == 1" />
            <Icon family="fas" name="caret-up" v-if="fullActionFilters.sort == 2" />
          </span>
          <span class="task-table-col-title task-channel-col">
            Channel
            <Icon family="fas" name="caret-down" v-if="fullActionFilters.sort == 3" />
            <Icon family="fas" name="caret-up" v-if="fullActionFilters.sort == 4" />
          </span>
          <span class="task-table-col-title task-due-col">
            Due Date
            <Icon family="fas" name="caret-down" v-if="fullActionFilters.sort == 5" />
            <Icon family="fas" name="caret-up" v-if="fullActionFilters.sort == 6" />
          </span>
        ...
        <div v-if="root && loading">
          <ActionTableLoader loading v-for="i in 10" :key="`task-loader-${i}`" />
        </div>
        <div v-if="!loading" class="nested-task" :class="{ ['nested-task-root']: root }">
          <TaskTableView
            v-for="(action, index) in actions"
            :class="{ ['d-none']: hide }"
            :key="action.id"
...
export default {
  ...
  methods: {
    ...
    onToggleExpand(event) {
      ...
      _action.meta.expanded = forceState !== null ? forceState : !_action.meta.expanded;
    ...
    async endDrag(event) {
      ...
      if (event.pullMode === "clone") {
        const parent_id = event.srcElement.getAttribute("data-action-parent-id"); //nested-task from where the element come;
      ...
      if (this.isTabContext || this.isGridContext || this.isGridSubtaskContext) {
        const channelActions = this.$store.getters["actions/getActionsByChannel"](channel_id);
        let newIndex = 0;
        const fromRoot = event.srcElement.classList.contains("nested-task-root");
        ...
        if (to.classList.contains("nested-task-root")) {
          ...
          const after = newIndex - 1 >= 0 ? channelActions[newIndex - 1].id : null;
        ...
        //to nested
          ...
          if (!fromRoot && event.oldIndex === event.newIndex && event.from == event.to) return; //moving to same position

          const under = to.id;
          const underAction = channelActions.find((action) => action.id == under);
          underAction.meta.expanded = true;
          const after = event.newIndex - 1 >= 0 ? underAction.children[event.newIndex - 1].id : null;
          response = await this.$store.dispatch("actions/move", {
            action_id,
            under,
            after,
    ...
    moveCallback(event) {
      ...
      const isRootWithChildren = event.draggedContext.element.children.length > 0;

      const movingToNestedTask = !event.to.classList.contains("nested-task-root");
    ...
    pullCallback(e) {
      if ((this.isGridContext || this.isGridSubtaskContext) && e.options.group.name === "action-tab") return "clone";
  ...
  components: {
    ...
    Loader,
    draggable,
    ActionTabLoader: () => import("@/components/ActionTabLoader"),
    ActionTableLoader: () => import("@/components/ActionTableLoader"),
    ActionGridLoader: () => import("@/components/ActionGridLoader"),
