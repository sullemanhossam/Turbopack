<template>
  ...
  >
    ...
    <Popover :ref="`duplicate-task-popover`" @apply-hide="isPopoverOpened['duplicate-task'] = false" @show="isPopoverOpened['duplicate-task'] = true">
      <button
        v-tooltip="{
          delay: 1000,
          offset: -5,
      ...
      <template slot="popover">
        <div class="duplicate-task-wrapper" v-if="isPopoverOpened['duplicate-task']">
          <template>
            <div class="dropdown-items">
              <button v-close-popover class="dropdown-item" type="button" @click="() => duplicateAction(action, 0)">
                <Icon family="fal" name="object-group" />
                {{ $t("actions.duplicate") }}
              </button>
              <button v-close-popover class="dropdown-item" type="button" @click="duplicateAction(action, 1)">
                <Icon family="fal" name="clone" />
                {{ $t("actions.copy-to-channel") }}
              </button>
              <button v-close-popover class="dropdown-item" type="button" @click="duplicateAction(action, 2)">
    ...
    >
      ...
      <template slot="popover">
        <AddWho
          ...
          @save="(e) => updateAction(action, e, 'members', () => hidePopover(`member-task-popover`))"
        />
      </template>
    </Popover>
    ...
    >
      ...
      <template slot="popover">
        <AddDueDate
          ...
          @save="(e) => updateAction(action, e, 'duedate', () => hidePopover(`duedate-task-popover`))"
        />
      </template>
    </Popover>

    <Popover :ref="`options-task-popover`" @apply-hide="isPopoverOpened['options-task'] = false" @show="isPopoverOpened['options-task'] = true">
      <button
        v-tooltip="{
          delay: 1000,
          offset: -5,
      ...
      <template slot="popover">
        <div class="dropdown-items" v-if="isPopoverOpened['options-task']">
          ...
          <button v-close-popover class="dropdown-item" v-if="root" type="button" @click="duplicateAction(action, 2)">
            <Icon family="fal" name="arrow-alt-right" />
            {{ $t("actions.move-to-channel") }}
          </button>
          <button v-close-popover class="dropdown-item d-none" type="button" disabled>
            <Icon family="fal" name="line-columns" />
            {{ $t("actions.change-type") }}
          </button>
          <button v-close-popover class="dropdown-item" type="button" @click="() => showActionModal(action)">
          ...
          <Popover class="header-option-delete-wrapper">
            ...
            <template slot="popover">
              <div class="dropdown-items">
                ...
                <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click="() => onDeleteAction(action)">
