<template>
  <div class="task-activities">
    ...
    <div v-for="(activity, index) in activities" :key="index" class="task-activity chat-fragment">
      <ChatMessage
      ...
      <div class="chat-message" v-else>
        ...
        <div class="activity-content">
          <div class="activity-header">
            ...
            <span class="when" v-tooltip="{ content: activity.created_on }">{{ parseTime(activity.created_on).fromNow() }}</span>
          </div>
          <div class="activity-body">
            ...
            <div v-else-if="activity.history_type === 'completed'">
              <span v-html="parseMessageContent($t('actions.activities.completed'))" />
            </div>

            <div v-else-if="activity.history_type === 'uncompleted'">
              <span v-html="parseMessageContent($t('actions.activities.uncompleted'))" />
            </div>
...
export default {
  ...
  methods: {
    ...
    onJumpTo(message) {
      this.$modal.hide("action-create");
      this.$router.push(`/channel/${message.channel_id}/messages/${message.id}`);
    },
  },
};
</script>
