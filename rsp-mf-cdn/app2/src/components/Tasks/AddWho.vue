<template>
  <div class="add-who-task">
    ...
    <div class="form-group" :class="{ [`is-invalid`]: errors['user_ids'] }">
      <label for="user_ids" class="mb-2 optional add-who-text">{{ $t("actions.forms.action-add-who") }}</label>
      <AutoComplete
        ref="autocomplete"
        :data="members"
    ...
    <div class="form-group chip-group">
      <div v-for="(user, index) in this.newUsers" :key="index" class="chip chip-sm">
    ...
    <button class="btn btn-primary btn-rounded mx-auto d-block" :disabled="unmodifiedUsers || disabled" @click="$emit('save', newUsers)">
