...
export default {
  ...
  methods: {
    onDragOverSubtaskArea: _.throttle(function (e) {
      ...
      var rect = {};
      if (this.$refs["subtask-drag-area"]) rect = this.$refs["subtask-drag-area"].getBoundingClientRect();
      if (e.clientY > rect.top && e.clientY < rect.bottom) this.$store.commit("actions/setSubtaskDragContext", true);
