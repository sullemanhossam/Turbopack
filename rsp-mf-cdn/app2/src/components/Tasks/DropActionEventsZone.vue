<template>
  <div class="drop-actions-events-wrapper" v-show="isDragging || isUsingDragZone">
    <transition name="actionEventsAnimation">
      <div class="drop-actions-events-body" v-if="isDragging || isUsingDragZone">
        <div class="drop-actions-events">
          <draggable v-bind="shareOptions" v-model="shareTasks" class="action-dropzone action-share-zone">
            <Icon family="fas" name="share-alt" />
            <span>Delete</span>
          </draggable>
          <Popover ref="trash-popover" class="action-dropzone">
            <draggable v-bind="trashOptions" v-model="trashCan" class="action-dropzone action-trash-zone" @add="(e) => addCallback(e, 'trash')">
...
export default {
  ...
  methods: {
    addCallback(event, type) {
      const { item } = event;
      const actions = this.$store.getters["actions/getActionsByChannel"](this.fullActionFilters.channel_id);
