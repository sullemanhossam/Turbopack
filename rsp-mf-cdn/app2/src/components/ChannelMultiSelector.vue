<template>
  <div>
    <div class="form-group" :class="{ [`is-invalid`]: errors['channels'] }">
      ...
      <small v-if="errors['channels']" class="form-text text-danger">{{ errors["channels"] }}</small>
    </div>
    ...
    <div v-if="value.length > 0" class="form-group chip-group mb-0">
      <div v-for="(channel, index) in value" :key="index" class="chip">
        ...
        <button type="button" name="button" class="btn btn-icon" @click="onRemoveChannel(index)">
...
import AutoComplete from "@/components/GenericAutoComplete.vue";
