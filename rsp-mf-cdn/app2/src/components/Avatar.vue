<template>
  ...
  >
    ...
    >
      ...
      >
        <span><Icon family="far" :name="optionsCount > 0 ? 'ellipsis-h' : 'pencil-alt'" /></span>
        <template slot="popover">
          <div class="dropdown-items">
            <button class="dropdown-item" type="button" v-close-popover @click="chooseFile" v-if="editable">
              <Icon family="far" name="upload" />
              <a>{{ $t("settings.profile.upload-profile-picture") }}</a>
            </button>
            <button class="dropdown-item" type="button" v-close-popover @click="discardChanges" v-if="editable && edited">
              <Icon family="fas" name="undo" />
              <a>{{ $t("settings.profile.discard-profile-picture-changes") }}</a>
            </button>
            <button class="dropdown-item" type="button" v-close-popover @click="onRemove" v-if="editable && removable && image">
  ...
  computed: {
    ...
    image: function () {
      ...
      if (this.channelId && this.hasAvatar) {
        return `${process.env.VUE_APP_ROOT_API}/v1/channel.image.get?channel_id=${this.channelId}&channel_asset_type=${this.type}&token=${
          hash && !this.privateOnly ? hash : store.get("jwt")
        }&v=${this.cache}`;
      }
      if (this.hasAvatar) {
        return `${process.env.VUE_APP_ROOT_API}/v1/user.image.get?${qs.stringify({
          user_id: this.userId,
          image_class: this.size,
          token: hash && !this.privateOnly ? hash : store.get("jwt"),
          call_hash: this.isMeeting ? this.$router.history.current.params.hash : undefined,
          v: this.cache,
        })}`;
      }
      return null;
    ...
    handleCroppa() {
      ...
      this.cropTimeout = setTimeout(() => {
        this.imageCropper.generateBlob(
          (blob) => {
            this.$emit("changed", new File([blob], "avatar.png", { type: "image/png" }));
          },
          "image/png",
