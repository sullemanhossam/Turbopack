<template>
  <Drag :transfer-data="item" class="file-explorer-single file-explorer-single--file">
    <template slot="image"
      ><div class="file-explorer-single-transfer"><FileIcon :name="item.name" :thumb="getThumb(item)" /> {{ item.name }}</div></template
    >
    <a class="file-explorer-single" @click="preview">
      ...
      <div class="file-explorer-single-title">
        ...
        <small v-else>{{ bytesToSize(item.size) }} - Last modified {{ parseTime(item.modification).format("MM/DD/YYYY") }}</small>
      </div>
...
export default {
  ...
  methods: {
    ...
    getThumb(file) {
      ...
      if (isImg(file.name)) {
        return `${process.env.VUE_APP_ROOT_API}/v1/file.thumb?fspath=${encodeURI(file.fullpath)}&width=100&height=100&token=${
