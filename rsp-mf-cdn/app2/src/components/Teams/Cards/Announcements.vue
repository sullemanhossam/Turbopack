<template>
  <div class="card card-float team-announcements">
    ...
    <div class="card-body" ref="card-body">
      <a class="card-overflow" v-if="isOverflowing" @click="onExpand">View all</a>
      <div class="announcement-content wiki-content" style="min-height: 0" ref="announcement" v-html="displayContent"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    displayContent() {
      ...
      return announcements && announcements !== "" ? markdownToHtml(announcements, "", true) : "";
    },
  },
  updated() {
  ...
  methods: {
    ...
    onUpdateContent() {
      if (
        ...
        this.$refs["announcement"].clientHeight > this.$refs["card-body"].clientHeight - 26
      ) {
