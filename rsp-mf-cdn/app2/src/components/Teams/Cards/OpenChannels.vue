  ...
  <div class="card card-float open-channels">
    ...
    <div class="card-body">
      <a class="card-overflow" v-if="isOverflowing" @click="onExpand">{{
        $tc("teams.settings.team-open-channels-more-channels", moreChannelsCount)
      }}</a>
      <ul class="details-list" v-if="splitedChannels.length">
        <li v-for="(channel, index) in splitedChannels" :key="index">
          <div class="channel-name overflow-hidden">
            <Avatar v-if="channel" :name="channel.name" type="logo" :channel-id="channel.id" :has-avatar="channel.has_logo" size="xs" light />
          ...
          <div class="channel-settings">
            <span>{{ $tc("channels.members-counter", channel.members_count) }}</span>
              ...
              <Popover>
                ...
                <template slot="popover">
                  <div class="dropdown-items">
                    <button v-close-popover class="dropdown-item" type="button" v-if="!channel.has_joined" @click="onJoinChannel(channel)">
                      {{ $t("teams.channel-public-options-join") }}
                    </button>
                    <button v-close-popover class="dropdown-item" type="button" v-else @click="onOpenChannel(channel)">
      ...
      <div v-else-if="user.teamRole.can('teams.manage')" class="empty-card">
        ...
        <small>{{ $t("teams.settings.team-open-channels-empty-description") }}</small>
...
export default {
  ...
  computed: {
    ...
    openChannels() {
      return this.team.open_channels ? _.orderBy(this.team.open_channels, "has_joined", "asc") : [];
    },
    isOverflowing() {
      return this.openChannels && this.openChannels.length > 3;
    },
    moreChannelsCount() {
      return (this.openChannels && this.openChannels.length) - (this.isOverflowing ? 2 : 3);
    },
  },
  methods: {
    onExpand() {
    ...
    async onJoinChannel(channel) {
      const response = await this.$store.dispatch("channels/joinOpenTeamChannel", {
