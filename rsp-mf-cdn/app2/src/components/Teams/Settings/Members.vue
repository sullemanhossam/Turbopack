<template>
  <div class="team-members members-list">
    ...
    <portal to="modal-subheader">
      <div class="teams-settings-subheader">
        <ExpansiveInput placeholder="Filter users..." v-model="filter">
          <button class="btn btn-link" @click="inviteUser" v-if="user.teamRole.can('settings.members.invite')">
            <Icon family="far" name="plus" />
            Invite user
          </button>

          <Popover popoverClass="sory-by-popover" container=".teams-settings-subheader">
            <button class="btn btn-link sort-by">
              <Icon family="far" name="sort-amount-down-alt" />
              <span v-if="sortBy === 'role-desc'" v-html="$t('teams.sort-by-role')"></span>
              <span v-else-if="sortBy === 'name-asc'" v-html="$t('teams.sort-by-name')"></span>
              <span v-else-if="sortBy === 'channels_count-asc'" v-html="$t('teams.sort-by-channels-count')"></span>
              <span v-else-if="sortBy === 'fs_size-asc'" v-html="$t('teams.sort-by-quota')"></span>
            </button>
            <template slot="popover">
              <div class="dropdown-items">
                <button v-close-popover class="dropdown-item" type="button" v-html="$t('teams.sort-by-role')" @click="sortBy = 'role-desc'"></button>

                <button v-close-popover class="dropdown-item" type="button" v-html="$t('teams.sort-by-name')" @click="sortBy = 'name-asc'"></button>
    ...
    <li v-for="member in sortedMembers" :key="member.id" class="members-list-item">
      <Avatar
        :name="member.name"
        :user-id="member.id"
      ...
      <fragment v-if="TeamRoles[member.role]">
        <Popover
          v-if="
            user.teamRole.can('settings.members.promote') && team.role_id >= member.role && member.id !== user.id && member.id !== team.created_by
          "
        >
            ...
            <div class="member-badge badge">
              {{
                team.created_by === member.id
                  ...
                  : $t(`teams.forms.members-badge-${TeamRoles[member.role].title}`)
              }}
            ...
            <div class="dropdown-items">
              ...
              >
                ...
                <Icon
                  v-tooltip="{
                    offset: 5,
                    content: $t(`teams.forms.members-badge-${role.title}-description`),
                  }"
                  family="fas"
                  name="info-circle"
                  class="icon-info"
        ...
        <div v-else class="members-list-item-role">
          <div class="member-badge badge">
            {{
              team.created_by === member.id ? $t("teams.forms.members-badge-owner") : $t(`teams.forms.members-badge-${TeamRoles[member.role].title}`)
            }}
          </div>
        </div>
      </fragment>
      ...
      <!-- Team channels -->
      <div class="members-list-item-team-channels" v-html="$tc('teams.settings.members-tab-team-channels', member.channels_count)" />
      ...
      <!-- Kick from channel -->
      <v-popover v-if="member.id !== user.id && user.teamRole.can('settings.members.kick') && team.role_id >= member.role" class="pl-3">
        ...
        <template slot="popover">
          <div class="dropdown-items">
            ...
            <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click="onRemoveMember(member.id)">
...
import ExpansiveInput from "@/components/ExpansiveInput.vue";
...
export default {
  ...
  methods: {
    ...
    async onRemoveMember(memberId) {
      ...
      const response = await this.$store.dispatch("teams/removeMember", payload);
