<template>
  <div>
    ...
    <form novalidate="true" @submit.prevent.stop>
      ...
      <AddMembers ref="addMember" :siblings="siblings" @updateEmails="(emails) => (form.emails = emails)" teamContext />

      <ChannelSelector v-model="form.selectedChannels" :channels="channelsAvailable" />

      <div class="form-group text-center pt-2">
        <button class="btn btn-primary btn-rounded px-4 mx-2" @click="onSubmitAddMembers" :disabled="!form.emails.length">
          {{ $t("general.add") }}
        </button>
        <button class="btn btn-primary btn-ghost mx-2" type="button" @click="$emit('return')">
          {{ $t("general.cancel") }}
        </button>
      </div>
    </form>
...
import AddMembers from "@/components/AddMemberAsEmail.vue";
import ChannelSelector from "@/components/ChannelMultiSelector.vue";
