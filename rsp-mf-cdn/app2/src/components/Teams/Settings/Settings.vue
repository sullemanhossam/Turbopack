<template>
  <div>
      ...
      <div class="section-body">
        ...
        <div class="general-usage-info">
          <div class="usage-info-item">
            ...
            <span class="key">{{ $t("teams.settings.settings-tab-usage-members") }}</span>
          </div>
          <div class="usage-info-item">
            <span class="value">{{ channelsCount }}</span>
            <span class="key">{{ $t("teams.settings.settings-tab-usage-channels") }}</span>
          </div>
          <div class="usage-info-item">
            <span class="value">{{ fileCount }}</span>
            <span class="key">{{ $t("teams.settings.settings-tab-usage-files") }}</span>
          </div>
        </div>
    ...
    <div class="section">
      <span class="section-title">{{ $t("teams.forms.settings-tab-team-name") }}</span>
      <div class="section-body">
        <input type="text" class="form-control" v-model="teamName" @input="hasUnsavedChanges = true" />
      </div>
    </div>

    <div class="section color-tag has-switcher">
      <span class="section-title">{{ $t("teams.forms.settings-tab-color-tag") }}</span>
      <div class="section-body">
        <div class="section-content">
          <span class="section-description">{{ $t("teams.forms.settings-tab-color-tag-description") }}</span>
          <ColorPalette :items="colors" v-model="color" @onAddColor="onAddColor" :disabled="!useColorTags" @input="onChangeColor" />
        </div>
        <div class="section-switcher">
          <ToggleSwitch v-model="useColorTags" :loading="loading['color_tag']" @input="onUseColorTagsInput" />
        </div>
      </div>
    </div>

    <div class="section announcements has-switcher">
      <span class="section-title">{{ $t("teams.forms.settings-tab-announcements") }}</span>
      <div class="section-body">
        <div class="section-content mr-2">
          <span class="section-description">{{ $t("teams.forms.settings-tab-announcements-description") }}</span>

          <textarea
            class="form-control announcements-textarea"
            :class="{ 'is-invalid': announcementsOverflow }"
            :placeholder="$t('teams.forms.settings-tab-announcements-placeholder')"
            ref="announcements"
          ...
          <div class="announcements-length-warning">
            <span :class="{ 'text-danger': announcementsOverflow }">{{ announcementsLength }}/{{ announcementsMaxLength }}</span>
          </div>
        </div>
        <div class="section-switcher">
          <ToggleSwitch v-model="useAnnouncements" :loading="loading.announcements" @input="onUseAnnouncementsInput" />
        </div>
      </div>
    </div>
    ...
    <div class="section privacy" v-if="false">
      ...
      <div class="section-body">
        <div class="section-content">
          ...
          <div class="section-item">
            <span class="item-name">Allow anyone to invite guests to team channels</span>
            <ToggleSwitch
          ...
          <div class="section-item">
            <span class="item-name">Allow any channel owner or channel admin to delete team channels</span>
            <ToggleSwitch
          ...
          <div class="section-item">
            <span class="item-name">Allow any guest to become admin or owner of a team channel</span>
            <ToggleSwitch
    ...
    <div class="section footer" v-if="hasUnsavedChanges">
      <button class="btn btn-primary" @click="onSaveChanges" :disabled="announcementsOverflow">Save</button>
    </div>
  </div>
</template>

<script>
import Progress from "@/components/ProgressCircular.vue";
...
export default {
  ...
  data() {
    return {
      color: "#FF6900",
      colors: ["#FF6900FF", "#FCB900FF", "#7BDCB5FF", "#00D084FF", "#8ED1FCFF", "#0693E3FF", "#ABB8C3FF", "#EB144CFF", "#F78DA7FF", "#9900EFFF"],
      useColorTags: true,
  ...
  computed: {
    usagePercent() {
      return this.fs_total > 0 ? ((this.fs_size / this.fs_total) * 100).toFixed(1) : 0.0;
    },
    fs_size() {
    ...
    usageLabel() {
      return `${this.bytesToSize(this.fs_size)} / ${this.bytesToSize(this.fs_total)}`;
    },
    announcementsLength() {
    ...
    channelsCount() {
      return (this.team && this.team.all_channels && this.team.all_channels.length) || 0;
    },
    membersCount() {
