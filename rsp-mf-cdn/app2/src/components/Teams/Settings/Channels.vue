<template>
  <div class="team-channels channel-list">
        ...
        <ExpansiveInput placeholder="Filter channels..." v-model="filter">
            ...
            <template slot="popover">
              <div class="dropdown-items team-filter-switcher">
                <div class="dropdown-title">
                  {{ $t("teams.settings.team-channels-create-new-channel-divider") }}
                </div>
                <button class="dropdown-item" @click="onCreateChannel" v-close-popover>
                  {{ $t("general.create-new-channel") }}
                </button>

                <div class="dropdown-divider"></div>
                <div class="dropdown-title">
                  {{ $t("teams.settings.team-channels-create-transfer-channel-divider") }}
                </div>
                <button
                  v-close-popover
                  class="dropdown-item"
          ...
          <Popover popoverClass="sory-by-popover" container=".teams-settings-subheader">
            <button class="btn btn-link sort-by">
              ...
              <span v-if="sortBy === 'display_name-asc'" v-html="$t('teams.sort-by-name')" />
              <span v-if="sortBy === 'total_file_size-desc'" v-html="$t('teams.sort-by-quota')" />
              <span v-if="sortBy === 'action_count-asc'" v-html="$t('teams.sort-by-actions-count')" />
            </button>
    ...
    <li v-for="channel in sortedChannels" :key="channel.id" class="channel-list-item">
      <Avatar
      ...
      <div class="display-name">
        ...
        <small v-html="$tc('teams.settings.channels-tab-members-count', channel.members && channel.members.length)" />
      </div>
      ...
      <!-- Team channels -->
      <div class="channel-list-item-files-count px-3" v-html="$tc('teams.settings.channels-tab-files-count', channel.total_fs_count)" />
      ...
      <!-- Actions count-->
      <div class="channel-list-actions-count px-3" v-html="$tc('teams.settings.channels-tab-actions-count', channel.action_count)" />

      <!-- Kick from channel -->
      <Popover class="pl-3" :autoHide="transferOwnershipPopoverChannel === null">
        ...
        <template slot="popover">
          <div class="dropdown-items">
            <button
              ...
              v-tooltip="{
                content: $t('teams.settings.channel-settings-open-channel-description'),
                delay: 500,
              }"
            >
...
import UsersChips from "@/components/UsersChips.vue";
...
import ChannelMembers from "@/components/Modals/ChannelMembers.vue";
