<template>
  <div class="chat-fragment-scroller-wrapper">
    <div class="chat-fragment-scroller" ref="scroller">
      ...
      />
      <ChatLoader ref="top-loader" v-if="channel.chat && (channel.chat.has_more || channel.chat.buffer.older.length > 0)" />
      <component
        ...
        :key="item.id"
      ></component>
      <ChatLoader ref="bottom-loader" v-if="channel.chat && (channel.chat.has_more_newer || channel.chat.buffer.newer.length > 0)" />
    </div>
    ...
    <div
      ...
      :class="{
        ['chat-fragment-scroller-floating--visible']: hasNewMessages || hasPersonTyping || hasPendingOneOne,
      }"
    >
    ...
    <div :class="{ active: loadingMore || isNavigating || isSyncing }" class="chat-fragment-scroller-loader">
      <span>{{ $t(isSyncing ? "channels.syncing-messages" : "channels.loading-messages") }}</span>
    </div>
  </div>
</template>
<script>
import ChatMessage from "@/components/ChatMessage.vue";
import ChatDivider from "@/components/ChatDivider.vue";
...
import OneOneApprovalMessage from "@/components/OneOneApprovalMessage.vue";
...
export default {
  ...
  computed: {
    ...
    hasPendingOneOne() {
      return this.channel.one_one && (this.channel.blocked_on || (!this.channel.one_one_approved && this.channel.owned_by !== this.user.id));
    },
    hasFloatingBar() {
      return this.hasNewMessages || this.hasPersonTyping || this.hasPendingOneOne;
    },
    user() {
  ...
  watch: {
    channel: {
      handler: function (newChannel, oldChannel) {
        if (!newChannel.chat.messages || this.messageCount !== newChannel.chat.messages.length) {
  ...
  beforeUpdate() {
    ...
    if (this.isNavigating === 1) {
      const firstMessage = this.$refs.scroller.querySelector(".chat-message:not(.chat-message--loader):not(.chat-message--sticky)");
      ...
      let lastMessage = this.$refs.scroller.querySelectorAll(".chat-message:not(.chat-message--loader):not(.chat-message--sticky)");
  ...
  updated() {
    this.$nextTick(function () {
      ...
      if (this.isNavigating !== 0 && this.pivotMessageId) {
        const pivotMessage = this.$refs.scroller.querySelector(`.chat-message[data-id="${this.pivotMessageId}"]`);

        if (
          this.isNavigating === 1 &&
          this.$refs.scroller.querySelector(".chat-message:not(.chat-message--loader):not(.chat-message--sticky)").getAttribute("data-id") !=
            this.pivotMessageId
        ) {
          this.$refs.scroller.scrollTop = pivotMessage.offsetTop - this.scrollDifference;
  ...
  methods: {
    init() {
      ...
      const target = this.$route.params.target ? this.$route.params.target : this.$route.query.highlight;
    ...
    handleScroll(e) {
      ...
      if (
        ...
        e.target.scrollTop + e.target.offsetHeight > this.$refs["bottom-loader"].$el.offsetTop &&
        !this.loadingMore
      ) {
        this.loadMore(true);
      }
    ...
    async loadMore(newer = false) {
      ...
      if (
        this.isOnBottom ||
        (!newer && !chat.has_more && chat.buffer && chat.buffer.older.length === 0) ||
        (newer && !chat.has_more_newer && chat.buffer && chat.buffer.newer.length === 0)
      ) {
    ...
    async scrollTo(id) {
      ...
      if (this.$refs[`message-${id}`] && this.$refs[`message-${id}`][0]) {
        ...
        this.isOnBottom = scroller.scrollTop + scroller.offsetHeight === scroller.scrollHeight ? true : false;
      } else {
        this.target = id;
      }
    },
