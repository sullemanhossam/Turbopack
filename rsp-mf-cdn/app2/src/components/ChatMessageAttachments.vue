<template>
  <div class="chat-message-content-attachment">
    ...
    <ul class="attachment-list">
      <li v-for="(file, index) in attachments.slice(0, attachments.length > 3 ? 2 : 3)">
        <a class="single-attachment" @click.prevent="preview($event, index)">
          ...
          <div class="single-attachment-info">
            ...
            <div class="progress" v-if="$store.state.files.progress[file.path] && loading">
              <div
                class="progress-bar"
                role="progressbar"
                v-bind:style="{
              ...
              <template slot="popover">
                <div>
                  <div class="dropdown-items">
                    <button class="dropdown-item" type="button" v-close-popover @click="() => onDownload(file)" :disabled="file.noDownload">
                      <Icon family="far" name="download" />
                      {{ $t("files.download") }}
                    </button>
                    <button class="dropdown-item" type="button" v-close-popover @click="preview($event, index)">
          ...
          <template slot="popover">
            <perfect-scrollbar class="attachment-list attachment-list-popover">
              <li v-for="(file, index) in attachments.slice(2)">
                <a class="single-attachment" v-close-popover @click="preview($event, index + 2)">
                  <FileIcon :name="file.file" :thumb="getThumb(file)" />
                  <div class="single-attachment-info">
                    <p>{{ file.file }}</p>
                    <small>{{ bytesToSize(file.size) }}</small>
                      ...
                      <template slot="popover">
                        <div>
                          <div class="dropdown-items">
                            ...
                            <button class="dropdown-item" type="button" v-close-popover @click="preview($event, index + 2)">
  ...
  methods: {
    ...
    getThumb(file) {
      ...
      if (isImg(file.file) || file.has_thumb) {
        return `${process.env.VUE_APP_ROOT_API}/v1/file.thumb?fspath=${encodeURIComponent(file.path)}&width=120&height=120&token=${token}&v=${
          this.version
        }`;
      }
    },
