  ...
  >
      ...
      <template slot="popover">
        <ul class="popover-emoji" @mouseleave="forcePopoverClose">
          <li
            ...
            v-bind:key="reaction"
            v-bind:class="{
              [`popover-emoji-item--picked`]: message.reactions[reaction] && message.reactions[reaction].indexOf(userId) > -1,
            }"
          >
      ...
      <template slot="popover">
        <div @mouseleave="forcePopoverClose">
          <div class="dropdown-items">
            <button class="dropdown-item" type="button" @click="onClickAction(1)">
    ...
    >
      ...
      <template slot="popover">
        <div @mouseleave="forcePopoverClose">
          <div class="dropdown-items">
            <button class="dropdown-item" type="button" @click="onEdit" v-if="message.content !== ''">
...
export default {
  ...
  methods: {
    ...
    onReact(reaction) {
      this.$emit("reacted", {
        reaction: reaction,
        id: this.message.id,
        type: !(this.message.reactions[reaction] && this.message.reactions[reaction].indexOf(this.userId) > -1),
      });
      this.forcePopoverClose();
    },
    onQuote() {
    ...
    forcePopoverClose() {
      if (this.current !== "") {
        this.$refs[`popover-${this.current}`] && this.$refs[`popover-${this.current}`].hide();
      }
      this.current = "";
    },
  },
  watch: {
    current() {
      if (this.current == "") this.$refs && this.$refs.actions.classList.remove("show");
