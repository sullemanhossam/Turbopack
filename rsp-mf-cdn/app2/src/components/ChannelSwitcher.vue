<template>
  <fragment>
    <aside class="channel-switcher" :class="{ [`channel-switcher--minified`]: isMinified }" v-hotkey="keymap">
      <div
        class="channel-switcher-toggle-button"
        :class="{
          ['has-unread']: $store.getters['channels/getUnreadCount'] > 0 && isMinified,
        }"
        @click="toggleMinification"
      >
        <Icon family="far" :name="isMinified ? 'arrow-to-right' : 'arrow-to-left'" />
      </div>
      ...
      <div class="channel-switcher-header">
        ...
        <Popover class="mr-auto" ref="createGroupPop" @update:open="createGroupOpened">
          <button
            v-tooltip="{
              delay: { show: 1000, hide: 0 },
              html: true,
              content: `<div class='create-channel-popup'><div>${$t('channels.groups.create-group')}</div><span>${$t(
          ...
          <template slot="popover">
            <Loader full :loading="$store.state.loading['channel.group.create']" />
            <div class="wrapper create-group-popup dropdown-items">
              <div class="form-group">
                <label for="channel_name" class="mb-2 title-text">{{ $t("channels.groups.create-group") }}</label>
                <input
                  id="channel_name"
                ...
                />
                <button class="btn btn-primary btn-sm" @click="onSubmitCreateGroup" :disabled="!newGroupName.trim()">
                  {{ $t("general.create") }}
                </button>
                <button class="btn ml-3 btn-sm btn-primary btn-ghost" @click="cancelGroupCreation">
                  {{ $t("general.cancel") }}
                </button>
              </div>
            </div>
      ...
      <div class="channel-switcher-subheader">
        <div class="form-group form-group--icon form-group--search">
          ...
          <input
            ...
            @keyup="onSearch"
          />
          <button v-if="query !== ''" type="button" class="btn btn-icon btn-sm" @click="onCancelSearch">
          ...
          <Popover popover-class="channel-switcher-settings">
            ...
            <template slot="popover">
              <div class="dropdown-items">
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('showGroups', true)">
                  <Icon family="fal" name="check" :class="{ active: showGroups }" />
                  {{ $t("channels.filters.show-categories") }}
                </button>
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('showGroups', false)">
                  <Icon family="fal" name="check" :class="{ active: !showGroups }" />{{ $t("channels.filters.hide-categories") }}
                </button>
                ...
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('sortBy', 'last_active_on_ts-desc')">
                  <Icon family="fal" name="check" :class="{ active: sortingByRecent }" />{{ $t("channels.filters.most-recently-active") }}
                </button>
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('sortBy', 'unread_count-desc')">
                  <Icon family="fal" name="check" :class="{ active: sortingByUnread }" />{{ $t("channels.filters.unread-count") }}
                </button>
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('sortBy', 'created_on_ts-desc')">
                  <Icon family="fal" name="check" :class="{ active: sortingByCreation }" />{{ $t("channels.filters.new-channels") }}
                </button>
                ...
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('filterBy', 'all')">
                  <Icon family="fal" name="check" :class="{ active: filteringByAll }" />{{ $t("channels.filters.show-all") }}
                </button>
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('filterBy', 'active')">
                  <Icon family="fal" name="check" :class="{ active: filteringByActive }" />{{ $t("channels.filters.show-active") }}
                </button>
                <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('filterBy', 'closed')">
                  <Icon family="fal" name="check" :class="{ active: filteringByClosed }" />{{ $t("channels.filters.show-closed") }}
                </button>
                ...
                <fragment v-if="useTeams">
                  ...
                  <button v-close-popover class="dropdown-item" type="button" @click="() => onChangeFilter('teams', [])">
                    <Icon family="fal" name="check" :class="{ active: showChannelsFromAllTeams }" />{{ $t("teams.forms.show-all") }}
                  </button>

                  <Popover trigger="hover" placement="left-start" popoverArrowClass="d-none" :offset="-12">
                    <button class="dropdown-item icon-padding dropdown-tree" type="button">
      ...
      <!-- Groups -->
      <perfect-scrollbar class="channel-switcher-body list-group" id="#channel-switcher-scroll-area" ref="viewport">
        <!-- Search results -->
        <div v-if="query.trim()" class="search-result">
          <div v-if="channels && channels.length === 0 && suggestedChannels.length === 0" class="empty-wrapper">
            <div class="empty-box">
              <Icon family="fal" name="filter" />
              <h4>{{ $t("channels.switcher-no-channels") }}</h4>
              <p>{{ $t("general.try-another-keyword") }}</p>
              <button class="btn btn-primary mx-auto" @click="onShowAllChannels">
                {{ $t("channels.show-all") }}
              </button>
            </div>
          </div>
                  ...
                  <div class="join-meeting">
                    <button
                      v-tooltip="{
                        delay: 1000,
                        offset: -5,
                        content: $t('channels.sidebar.start-a-new-conversation-tooltip'),
                      }"
        ...
        <div v-else-if="showGroups" class="vue-frag">
          ...
          <div v-if="!hasChannelsAfterFilter" class="empty-wrapper">
            <div class="empty-box">
              ...
              <h4>{{ $t("channels.switcher-no-channels") }}</h4>
              <p>
                {{ $t(`home.filter-not-found-${userPreferences.listing.filterBy}`) }}
              </p>
              <button class="btn btn-primary mx-auto" @click="onShowAllChannels">
                {{ $t("channels.show-all") }}
              </button>
          ...
          >
            <div class="channel-group--name d-flex align-items-center" @click="toggleExpandedGroup(group)">
              <Icon name="chevron-right" class="channel-name-direction" v-if="!group.fixed" />
              <span class="name">
                {{ group.virtual ? $t(`channels.groups.virtual-${group.id}`) : group.name }}
              </span>
              <UnreadBadge class="ml-3" v-if="!group.expanded" :unread_count="group.unread_count" />
              <div class="more-options ml-auto" v-if="!group.fixed" @click.prevent.stop>
                <Popover>
                  <button
                    v-tooltip="{
                      delay: 1000,
                  ...
                  <template slot="popover">
                    <div class="dropdown-items">
                      <button v-if="!group.virtual" v-close-popover class="dropdown-item" type="button" @click="showAddChannelsGroup(group)">
                        <Icon family="fal" name="plus-circle" />
                        {{ $t("channels.groups.add-or-remove-channels") }}
                      </button>
                      <button v-if="!group.virtual" v-close-popover class="dropdown-item" type="button" @click="showRenameGroup(group)">
                        <Icon family="fal" name="money-check-edit" />
                        {{ $t("channels.groups.rename-group") }}
                      </button>

                      <button v-if="index != 0" v-close-popover class="dropdown-item" type="button" @click="moveGroup(group, 'up')">
                        <Icon family="fal" name="chevron-up" />
                        {{ $t("channels.groups.move-up") }}
                      </button>

                      <button v-if="index < groups.length - 1" v-close-popover class="dropdown-item" type="button" @click="moveGroup(group, 'down')">
                      ...
                      <Popover v-if="!group.virtual">
                        ...
                        <template slot="popover">
                          <div class="dropdown-items">
                            ...
                            <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click.prevent.stop="onSubmitDelGroup(group)">
            ...
            <transition name="fade" mode="out-in">
              <div class="group-channels-body" v-if="group.expanded || group.fixed">
                <div v-if="!group.channels || group.channels.length === 0" class="group-channels-empty">
      ...
      <div class="unread-viewport" v-if="!query.trim()">
        <transition name="fade">
          <div class="unread-background" v-if="unreadMessagesBellowViewport > 0"></div>
        </transition>
        <transition name="slide">
          <div class="unread-bubble" v-if="unreadMessagesBellowViewport > 0">
            <span @click="jumpToFirstUnreadChannel"
              ><AnimatedNumber :number="unreadMessagesBellowViewport" :i18n="'channels.sidebar.unread-messages-below'"
            /></span>
          </div>
        </transition>
      </div>
...
import ToggleGroupCollapse from "@/assets/close_groups.svg";
import ToggleGroupExpand from "@/assets/open_groups.svg";
...
export default {
  data: function () {
    ...
    let isAlone = channels && user && channels.length === 1 && channels[0].created_by !== user.id;
    ...
    return {
      ...
      isMinified: store.get("isChannelSwitcherMinified") || isAlone ? true : false,
  ...
  computed: {
    ...
    hasChannelsAfterFilter() {
      if (this.userPreferences.listing.showGroups) {
        return this.groups.some((group) => group.channels && group.channels.length > 0);
      } else {
        return this.sortedChannels && this.sortedChannels.channels && this.sortedChannels.channels.length > 0;
      }
    },
    keymap() {
      return {
    ...
    toggleCollapseTooltip() {
      let key = "c";
      return `${this.$t("channels.switcher-group-collapse")} (${this.optionKeyStr} + shift + ${key})`;
    },
    loading() {
    ...
    suggestedChannels() {
      if (this.query) {
        ...
        let directMessageUsers = _.filter(this.channels, { one_one: true }).map((channel) => {
  ...
  methods: {
    ...
    handleDrop(channel, e, group) {
      ...
      let fromGroupId = channel.channel_group_id != -1 ? channel.channel_group_id : channel.one_one ? "dm" : "all";
      ...
      } else if ((channel.one_one && group.id === "all") || (!channel.one_one && group.id === "dm")) {
    ...
    handleDropEnter(channel, e, group) {
      if ((channel.one_one && group.id === "all") || (!channel.one_one && group.id === "dm")) {
        //prevent moving a DM to ALL or Group to DM
        return;
      }
    ...
    onToggleTab() {
      this.selectedTab = this.selectedTab === "channels" ? "directMessages" : "channels";
    ...
    customChannels(id, returnAll = true) {
      ...
      return channels.filter((channel) => {
        ...
        return channel.channel_group_id == id && channel.one_one != (this.selectedTab == "channels");
      });
    },
    ...
    moveGroup(selectedGroup, direction) {
      ...
      let afterGroupId = direction === "up" ? (index == 1 ? 0 : groups[index - 2].id) : groups[nextGroupID].id;
    ...
    async onSubmitCreateGroup(e) {
      ...
      const response = await this.$store.dispatch("channels/createChannelGroup", {
    ...
    async onSubmitDelGroup(group) {
      const response = await this.$store.dispatch("channels/deleteChannelGroup", group);
    },

    async onSubmitRenameGroup({ name, channel_group_id }) {
      const response = await this.$store.dispatch("channels/renameChannelGroup", {
        channel_group_id,
        name,
      });
      if (response.ok) {
    ...
    handleIntersection(target) {
      ...
      if (target.classList.contains("channel-group")) {
        ...
        if (isBellowViewport) {
          ...
          if (this.scrollingToChannel == target.dataset.channel_id) this.scrollingToChannel = null;
          this.$store.dispatch("channels/setChannelsBellowViewport", {
            channel_id: target.dataset.channel_id,
            isBellowViewport: false,
            unreadCount: parseInt(target.dataset.unread_count),
    ...
    getFirstUnreadChannel() {
      ...
      if (this.userPreferences.listing.showGroups) {
        for (let group of this.groups) {
          ...
          if (group.expanded && Object.keys(group.unreadChannelsList).length > 0) {
        ...
        for (let channel of Object.values(this.sortedChannels.unreadChannelsList)) {
          if (this.channelsBellowViewport[channel.id] !== undefined) {
            //channel is bellow viewport
    ...
    jumpToFirstUnreadChannel() {
      ...
      if (this.userPreferences.listing.showGroups) {
        ...
        this.$nextTick(() => {
          ...
          let channelEl = this.$refs[`channel-list-${group.id}`][0].$refs[`${channel.id}-channel-item`][0];
          if (channelEl) {
            viewportEl.scrollTo({
        ...
        let viewportEl = this.$refs["viewport"].$el;
        let channelEl = this.$refs["channel-list"].$refs[`${channel.id}-channel-item`][0];
    ...
    updateObserver(newPreferences = null) {
      const preferences = newPreferences ? newPreferences : this.userPreferences;
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      for (let group of this.groups) {
        let el = this.$refs[`channel-group-${group.id}`] && this.$refs[`channel-group-${group.id}`][0].$el;
  ...
  watch: {
    groups() {
      ...
      this.$nextTick(() => {
        for (let group of this.groups) {
          let el =
            this.$refs[`channel-group-${group.id}`] && this.$refs[`channel-group-${group.id}`][0] && this.$refs[`channel-group-${group.id}`][0].$el;
          if (el) this.observer.observe(el);
        }
      });
    },
