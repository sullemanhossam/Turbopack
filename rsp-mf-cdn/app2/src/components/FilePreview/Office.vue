...
import { getFilePureBlob, getFileExtension, getFileType, loadScript } from "airsend/utils";
...
export default {
  ...
  methods: {
    async load() {
      ...
      if (["docx", "xlsx", "pptx"].indexOf(getFileExtension(this.file.file)) === -1) {
      ...
      const token = this.$route.query.hash ? this.$route.query.hash : this.$store.state.core.user.token;
      ...
      switch (getFileType(this.file.file)) {
        case "word":
          ...
          this.instance = window.docxJS = window.createDocxJS ? window.createDocxJS() : new window.DocxJS();
        ...
        case "excel":
          ...
          this.instance = window.cellJS = window.createCellJS ? window.createCellJS() : new window.CellJS();
        ...
        case "powerpoint":
          ...
          this.instance = window.slideJS = window.createSlideJS ? window.createSlideJS() : new window.SlideJS();
      ...
      this.instance.parse(this.blob, this.onParseDocument, this.onParseDocumentFail);
    },
    onParseDocument() {
