<template>
  <div class="file-preview-item file-preview-item--picture">
    <v-zoomer ref="image" :min-scale="0.5" @update:zoomed="onZoomed" :max-scale="5" style="width: 100%; height: 100%; min-height: 0">
      <template>
        <img :src="this.blob" v-show="!isLoading" @load="onLoadImage" @error="onError" />
  ...
  methods: {
    async loadBlob() {
      ...
      this.blob = this.file.noBlob
        ? this.file.path
        : getFileUrl(this.file.path.indexOf(".gif") > -1 ? "download" : "thumb", this.file.path, {
            width: 2560,
            height: 1440,
