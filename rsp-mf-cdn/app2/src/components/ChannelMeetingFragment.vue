  ...
  >
    ...
    >
      <div class="empty-wrapper" v-if="meeting.roomState === 'closed'">
        <div class="empty-box empty-box--meeting empty-box--boxed">
          ...
          <h4 class="text-center">
            {{ $t(`meeting.closed-${meeting.closeReason ? meeting.closeReason : "default"}-title`) }}
          </h4>
          <p class="text-center mb-0">
            {{ $t(`meeting.closed-${meeting.closeReason ? meeting.closeReason : "default"}-description`) }}
          </p>
          <button
            class="btn btn-link mx-auto mt-3"
            type="button"
            @click="$store.dispatch('meeting/downloadChat')"
            v-if="(!meeting.closeReason || meeting.closeReason === 'meeting-terminated') && meeting.chatHistory.length > 0"
          >
            <Icon name="download" />
            {{ $t("meeting.settings.general.download-chat-title") }}
          </button>
          <button class="btn btn-link btn-link--danger mx-auto mt-3" type="button" @click="$store.dispatch('meeting/close', true)">
      ...
      >
        <div class="empty-box empty-box--meeting empty-box--boxed">
          ...
          <div class="form-group form-section w-100 text-center mb-0" style="max-width: 600px">
            <input
              ref="meetingLink"
              class="form-control text-center my-3"
              readonly="readonly"
              @focus="onFocusLinkInput"
              :value="isElectron() ? `https://live.airsend.io/meeting/${meeting.roomId}` : location.href"
              v-if="$route.name === 'meeting'"
            />
            <button class="btn btn-link mx-sm-2 mt-md-1 mr-2" type="button" @click="onCopyLink">
          ...
          <button
            ...
            @click="$store.commit('meeting/set', { isMeetingInfoVisible: false })"
            type="button"
          >
            {{ $t("general.ok") }}
          </button>
      ...
      <div class="empty-wrapper" v-else-if="$route.name === 'meeting' && !user.display_name && !meeting.guest">
        <div class="empty-box empty-box--boxed empty-box--meeting">
          <form id="login" novalidate="true" @submit="onSubmitGuestForm">
            ...
            <div class="form-group" :class="{ [`is-invalid`]: errors['display_name'] }">
              <label for="display_name">{{ $t("meeting.public.display-name") }}</label>
              <input
                id="display_name"
                type="display_name"
                ...
                autofocus
              />
              <small v-if="errors['display_name']" class="form-text text-danger">{{
                $t(errors["display_name"].message, errors["display_name"].meta)
              }}</small>
            </div>

            <div class="form-group form-check">
              <input id="join_video" type="checkbox" class="form-check-input" v-model="form.join_video" />
              <label class="form-check-label" for="join_video">{{ $t("meeting.public.join-with-video") }}</label>
            </div>

            <div class="text-center d-block">
              <button type="submit" class="btn btn-half btn-primary btn-rounded btn-extended mx-auto d-block mt-4">
      ...
      <Portal to="meeting-menu-area">
        <div class="meeting-body-menu" v-if="user.id || meeting.guest">
          ...
          >
            <Icon family="far" :name="isExpanded ? 'compress-alt' : 'expand-alt'" />
          </button>
          <button
          ...
          >
            <Icon family="far" :name="isFullscreen ? 'compress-wide' : 'expand-wide'" />
          </button>
          <button
            v-if="meeting.peers"
            class="btn btn-icon"
      ...
      <transition-group class="meeting-body-toasts" name="meeting-toast" tag="ul">
        <li v-for="toast in meeting.toasts" :key="toast.id">
          <div
            class="toast-item"
            :class="[`toast-item--${toast.type}`]"
            v-html="typeof toast.content === 'string' ? toast.content : $t(toast.content.message, toast.content.meta)"
          ></div>
        </li>
      </transition-group>
    </div>
    ...
    <div class="meeting-footer" v-if="user.id || meeting.guest">
      ...
      <button
        ...
        @click="toggleMic"
      >
        <Icon family="far" :name="meeting.micProducer && !meeting.micProducer.paused ? 'microphone' : 'microphone-slash'" />
      </button>
      <button
        class="btn btn-icon"
        :disabled="meeting.roomState !== 'connected'"
        ...
        @click="toggleWebcam"
      >
        <Icon family="far" :name="meeting.webcamProducer && !meeting.webcamProducer.paused ? 'video' : 'video-slash'" />
      </button>

      <v-popover
        v-if="isElectron() && meeting.capabilities.canShareScreen && meeting.roomState !== 'closed'"
      ...
      >
        <button
          ...
          :disabled="meeting.roomState !== 'connected'"
          :class="{
            active: meeting.screenSharingProducer && !meeting.screenSharingProducer.paused,
        ...
        <template slot="popover">
          <div
            ...
            :set="
              (currentScreen =
                meeting.settings.selectedScreen && meeting.screens[meeting.settings.selectedScreen]
                  ? meeting.settings.selectedScreen
                  : Object.keys(meeting.screens)[0])
            "
          >
            <img
              v-if="meeting.screens[currentScreen] && meeting.screens[currentScreen].hasImage"
              :src="meeting.screens[currentScreen].image"
              class="thumbnail"
            />
            <div class="form-group">
              <label for="preferred_screen" class="mb-2">{{ $t("meeting.screenshare-choose-window") }}</label>
              <select id="preferred_screen" class="form-control" :value="currentScreen" @change="onChangeDevice($event, 'screen')">
                <option v-for="screenId in Object.keys(meeting.screens)" :key="screenId" :value="screenId">
                  {{ meeting.screens[screenId].name }}
                </option>
              </select>
            </div>
            <button
              v-if="meeting.screenSharingProducer && !meeting.screenSharingProducer.paused"
              class="btn btn-danger btn-block"
              @click="toggleScreenSharing"
            >
              {{ $t("meeting.screenshare-stop") }}
            </button>
            <button v-else class="btn btn-primary btn-block" @click="toggleScreenSharing">
      ...
      <button
        v-else-if="meeting.capabilities.canShareScreen && meeting.roomState !== 'closed'"
        :disabled="meeting.roomState !== 'connected'"
        class="btn btn-icon"
        :class="{
          active: meeting.screenSharingProducer && !meeting.screenSharingProducer.paused,
      ...
      <button class="btn btn-icon danger" v-if="meeting.roomState !== 'closed'" @click="closeMeeting">
...
export default {
  ...
  watch: {
    ...
    "meeting.peers"(peers, previous_peers) {
      if (this.pinned && this.pinned.pin && this.pinned.pin !== this.selfPeer.id) {
        const pinnedInCall = this.peers.some((peer) => peer.id === this.pinned.pin);
  ...
  computed: {
    ...
    peersListLength() {
      return this.screenSharePeer.length + this.peers.length + this.showSelfPeer;
    },
    selfPeerSpotlight() {
    ...
    screenSharePeer() {
      const peersScreensharing = _.values(this.videoConsumers.map((consumer) => consumer.peerId));
      let peers = this.meeting.peers.filter(
        (peer) => peersScreensharing.includes(peer.id) && (this.isGridView || !(this.spotlight.screenShare && peer.id === this.spotlight.id))
      );

      if (!!this.producers.screen && (this.isGridView || !this.hasSpotlight(this.selfPeer.id, true))) {
    ...
    sortedPeers() {
      ...
      return this.peers.sort((a, b) => {
        return (this.meeting.peerActivities[b.id] || 0) - (this.meeting.peerActivities[a.id] || 0) + 5;
      });
    },
    selfPeer() {
    ...
    peers() {
      if (this.showSpotlight && this.spotlight && this.showSpotlight)
        return this.meeting.peers.filter((peer) => !(!this.spotlight.screenShare && peer.id === this.spotlight.id));
    ...
    fitsNormalPeerSize() {
      const maxPeersSizeWithoutScroll = this.gridMeta.peersPerRow * this.gridMeta.maxRows;
  ...
  methods: {
    hasSpotlight(id, screenShare) {
      return screenShare === this.zScreenShareSpotlight && id == this.spotlight.id;
    },
    evaluateMaxPeersInPage() {
      const gridElm = this.$refs["meeting-grid"] && this.$refs["meeting-grid"].$el;
      if (!gridElm) return;
    ...
    evaluatePeersSort() {
      ...
      this.peers.forEach((peer, index) => {
        if (index < 2) {
          ...
          //sort others
          if ((this.meeting.peerActivities[peer.id] || 0) > this.meeting.peerActivities[_peers[0].id]) {
    ...
    async onChangeDevice(e, type) {
      ...
      if (this.meeting.screenSharingProducer && !this.meeting.screenSharingProducer.paused) {
    ...
    pin({ pin, screenShare }) {
      ...
      if (this.pinned.pin === pin && this.pinned.isScreenShare === screenShare) {
        this.pinned = {};
      } else {
    ...
    onSubmitGuestForm(e) {
      ...
      const isValid = this.$store.state.core.client.validator.validate(this.form, {
        display_name: {
          type: "string",
          empty: false,
        },
