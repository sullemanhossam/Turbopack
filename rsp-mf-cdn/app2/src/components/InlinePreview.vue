<template>
  <div class="file-preview file-preview--inline" v-bind:class="{ ['file-preview--active']: isActive }">
      ...
      <a class="navbar-icon has-popover" v-if="transactions.length > 0">
        <Popover popoverClass="popover-large">
          <a
            class="navbar-icon"
            v-bind:class="{
              ['navbar-icon--alert']: runningTransactions.length > 0 || failedTransactions.length > 0,
    ...
    <div class="file-preview-portal">
      <component v-bind:is="currentView" :file="currentFile" :edit="edit" @download="onDownload" @edit="onEdit" @fallback="onFallback" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    currentView() {
      ...
      // media files
      if (["pdf", "img", "audio", "video", "txt"].indexOf(type) > -1) {
