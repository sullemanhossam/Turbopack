<template>
  <div v-if="(meeting.roomId && meeting.roomState !== 'closed') || (currentChannel && currentChannel.meeting)" class="meeting-in-progress">
    <div class="meeting-name">
      {{ channel && channel.channel_name ? channel.channel_name : $t("meeting.meeting-in-progress") }}
    </div>
      ...
      <v-popover>
        ...
        <template slot="popover">
          <div class="dropdown-items">
            ...
            <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click="$store.dispatch('meeting/close')">
...
export default {
  ...
  computed: {
    ...
    channel() {
      return this.$store.getters["channels/getChannelById"](this.meeting.channelId);
    },
    isMeetingActive() {
      return this.meeting.roomId && this.meeting.roomState !== "closed";
    },
    currentChannel() {
      return this.$route.params.id ? this.$store.state.channels.single[this.$route.params.id] : null;
    },
  },
  methods: {
    expand() {
