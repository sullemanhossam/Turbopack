<template>
  <div class="meeting-body-spotlight" @click="details">
    ...
    >
      <MeetingVideo v-if="spotlight.screenShare && consumers.screen" :consumer="consumers.screen" />
      <MeetingVideo v-else-if="consumers.webcam" class="cover" :consumer="consumers.webcam" />
      <Avatar
      ...
      />
      <div
        v-if="consumers.mic && !consumers.webcam && !spotlight.screenShare && (selfSpotlight ? !consumers.mic.paused : !consumers.mic.remotelyPaused)"
        class="speaking-level"
        :style="{
          transform: `scale(${(1 + Math.round(meeting.peerVolumes[peer.id]) / 100).toFixed(2)})`,
        }"
      ></div>
...
export default {
  ...
  computed: {
    isScreenShareSpotlight() {
      return this.consumers.screen && (this.pinned.screenShare || this.pinned.screenShare === undefined);
    },
    screenshare() {
    ...
    spotlight() {
      //pinned
      if (this.pinned && this.pinned.pin && (this.pinned.pin === this.selfPeer.id || this.getPeer(this.pinned.pin))) {
        this.$emit("input", {
          id: this.pinned.pin,
      ...
      //last active speaker
      if (this.meeting.lastActiveSpeaker && this.getPeer(this.meeting.lastActiveSpeaker)) {
        this.$emit("input", {
    ...
    peer() {
      ...
      return this.spotlight.id ? this.$store.getters["meeting/getPeer"](this.spotlight.id) : {};
    },
    consumers() {
      if (this.selfSpotlight) return this.producers;

      return this.spotlight.id ? this.$store.getters["meeting/getConsumerByType"](this.spotlight.id) : {};
    },
    producers() {
