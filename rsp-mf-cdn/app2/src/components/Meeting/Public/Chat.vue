<template>
  ...
  >
    <div class="meeting-chat-fragment-body" ref="scroller">
      ...
      <div class="empty-wrapper" v-if="history.length === 0 && meeting.roomState !== 'closed'">
      ...
      <div class="single-message" :class="{ 'single-message--mine': meeting.peerId == message.author.id }" v-for="(message, index) in history">
        <Avatar
          size="medium"
          :name="message.author.display_name"
          :user-id="typeof message.author.id === 'number' ? message.author.id : null"
        ...
        <div class="message-wrapper">
          <h4>
            {{ meeting.peerId == message.author.id ? $t("meeting.you") : message.author.display_name }}
            -
          ...
          <div class="message-content" v-else v-html="message.rich_content"></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  destroyed() {
    if (this.$refs.scroller) this.$refs.scroller.removeEventListener("scroll", this.onScroll);
  },
  methods: {
    onScroll(e) {
