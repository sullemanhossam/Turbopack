<template>
  <div class="file-explorer file-explorer-files" v-if="isFilesContext || isGridFilesContext">
      ...
      <v-popover v-if="user.role.can('file.upload')">
        ...
        <template slot="popover">
          <div class="dropdown-items">
            <button v-close-popover class="dropdown-item" type="button" @click="() => this.$refs.fileList.openCreateFolder()">
              <Icon family="fal" name="folder-plus" />
              {{ $t("files.more-options.file-explorer-header-create-folder") }}
            </button>
            <button v-close-popover class="dropdown-item" type="button" @click="onDownloadFolder">
    ...
    <Modal name="full-manager" title="Files" class="full-modal-explorer" @before-open="onOpenModal" @closed="onCloseModal">
      <FullFileExplorer v-if="isModalOpen" :initial-path="currentPath" :no-history="true" />
    </Modal>
  </div>
  <div class="file-explorer file-explorer-media" v-else-if="isMediaContext">
    <GridFileList
      ...
      @paginate="fetchPage"
    />
    <Modal name="full-manager" title="Files" class="full-modal-explorer full-file-explorer-modal" @before-open="onOpenModal" @closed="onCloseModal">
      <FullFileExplorer v-if="isModalOpen" :initial-path="currentPath" :no-history="true" />
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    async load() {
      ...
      this.currentPath = pathMatch ? `${this.initialPath}/${pathMatch}` : this.initialPath;
