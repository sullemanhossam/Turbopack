<template>
  <div class="file-list-wrapper file-list-wrapper--grid">
    ...
    <Drop v-cloak class="droppable" @drop="onDropFile" @dragenter="onDragEnter" @dragleave="onDragLeave">
      <div class="file-list-body" :class="{ [`is-dragging`]: isDragActive }" id="infinite-list">
        ...
        <div>
          <div class="file-list-grid" id="file-list-grid">
            ...
            <fragment v-if="pathInfo.hasPageAfter && !loading">
              <GridItemLoader v-for="i in remainingCellsInLastRow" :key="`${i}-loader-fill-row`" />
              <GridItemLoader
                ref="item-loader"
            ...
            <fragment v-else>
              <div v-for="i in remainingCellsInLastRow" :key="`${i}-fill-row`"></div>
            </fragment>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    files() {
      if (this.media) return this.$store.getters["files/getMediaInPath"](this.currentPath);
    ...
    pathInfo() {
      if (this.media) return this.$store.getters["files/getPathInfo"](`${this.currentPath}/media`);
    ...
    loading() {
      return this.$store.state.loading[`file.list/loading-${this.currentPath}-${this.filesContext.type}`];
    },
    loadingAfter() {
    ...
    loaderItems() {
      ...
      let numberOfRows = Math.ceil((itemsPerPage + 3 * this.itemsPerRow) / this.itemsPerRow); //number of columns required + 3
