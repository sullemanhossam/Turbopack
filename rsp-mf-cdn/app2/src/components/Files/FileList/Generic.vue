  ...
  >
    <template slot="image">
      <div class="file-explorer-single-transfer">
        ...
        {{ item.displayname }}
        <div v-if="this.selection.length > 1" class="file-explorer-single-transfer-badge">+{{ this.selection.length - 1 }}</div>
      </div>
    </template>
    <th v-if="hasMultiSelect" scope="row" class="file-explorer-checkbox" @click="select">
    ...
    <td @click="preview">
      <div class="file-info">
        ...
        <h4 v-else>
          {{ item.displayname }}
          <div v-if="$store.state.files.progress[item.fullpath] && uploading" class="progress">
            <div
              class="progress-bar"
              role="progressbar"
              :style="{
    ...
    <td v-if="!selectOnly && !simplified" class="hover-only d-none d-sm-table-cell">
      <div class="btn-group">
        <button
        ...
        <Popover v-if="item.candelete" @apply-hide="isoptionsOpened = false" @show="isoptionsOpened = true" class="btn btn-sm btn-icon">
          <Icon
            family="fal"
            name="trash-alt"
            v-tooltip="{
    ...
    <td v-if="!selectOnly" class="hover-only text-right">
      <Popover @apply-hide="isoptionsOpened = false" @show="isoptionsOpened = true" v-if="item.candownload || item.canmove || item.candelete">
        <button
          v-tooltip="{
            delay: 1000,
            offset: -5,
...
export default {
  ...
  computed: {
    ...
    thumb() {
      let file = this.item;
      const token = this.$route.query.hash ? this.$route.query.hash : this.user.token;
      ...
      if (this.renamedFileProps) {
        ...
        return isImg(file.name)
          ? `${process.env.VUE_APP_ROOT_API}/v1/file.thumb?fspath=${encodeURI(this.renamedFileProps.from)}&width=100&height=100&token=${token}&v=${
              file.modificationts
            }`
          : null;
      }
      ...
      return isImg(file.name)
        ? `${process.env.VUE_APP_ROOT_API}/v1/file.thumb?fspath=${encodeURI(file.fullpath)}&width=100&height=100&token=${token}&v=${
            file.modificationts
          }`
        : null;
    },
