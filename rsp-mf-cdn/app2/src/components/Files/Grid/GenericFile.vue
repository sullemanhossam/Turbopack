  ...
  >
    ...
    <div class="item-footer" @mouseenter="onMouseOver">
      <div class="title-wrapper" ref="file-name-container">
        <span :class="{ [`text-overflow`]: textOverflow }">{{ item.name }}</span>
      </div>
      <Popover @apply-hide="isoptionsOpened = false" @update:open="isoptionsOpened = true" v-if="hoverActive || isSelected || isoptionsOpened">
        <button
          v-tooltip="{
            delay: 1000,
            offset: -5,
        ...
        <template slot="popover">
          <div class="dropdown-items">
            <button v-if="media" v-close-popover class="dropdown-item" type="button" @click="showInFolder(item)">
  ...
  computed: {
    ...
    thumb() {
      ...
      if (this.renamedFileProps) {
        ...
        if (isImg(file.name)) {
          return `${process.env.VUE_APP_ROOT_API}/v1/file.thumb?fspath=${encodeURI(
