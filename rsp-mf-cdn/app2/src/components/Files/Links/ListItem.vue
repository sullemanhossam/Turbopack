<template>
  ...
  >
    <div class="link-image">
      <ImageComponent :src="thumb" v-if="thumb && !thumbError" :key="thumb" @error="thumbError = true" />
      <ImageComponent
    ...
    <div class="link-body">
      ...
      <div class="description">
        <span v-if="messageContent">{{ messageAuthor }}: {{ messageContent }}</span>
        <span v-else>{{ $t("links.author-sent-link", { display_name: messageAuthor }) }}</span>
      </div>
      <div class="link">
        <img v-if="item.url_data.favicon && !favError" class="favicon" :src="item.url_data.favicon" alt="" @error="favError = true" />
        <span>{{ item.url_data.url }}</span>
      </div>
    </div>
    <Popover @apply-hide="isoptionsOpened = false" @show="isoptionsOpened = true" v-if="hoverActive || isoptionsOpened">
      <button
        v-tooltip="{
          delay: 1000,
          offset: -5,
      ...
      <template slot="popover">
        <div class="dropdown-items">
          <button v-close-popover class="dropdown-item" type="button" @click="onJumpTo(item.message)">
            <Icon family="fal" name="comment-alt-dots" />
            {{ $t("links.more-options.item-show-in-chat") }}
          </button>
          <button v-close-popover class="dropdown-item" type="button" @click="openLink(item.url_data.url)">
...
export default {
  ...
  computed: {
    thumb() {
      if (this.item.url_data.images && this.item.url_data.images.length > 0 && this.item.url_data.images[0][0]) {
    ...
    messageAuthor() {
      return this.$store.getters["channels/getUserInChannel"](this.channel.id, this.item.message.user_id).display_name;
    },
    itemTitle() {
      return this.item.url_data.title ? this.item.url_data.title : this.getUrlDomain(this.item.url_data.url);
    },
  },
  methods: {
    openLink(url) {
