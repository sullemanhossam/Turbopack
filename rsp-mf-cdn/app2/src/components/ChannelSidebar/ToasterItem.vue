  ...
  <fragment>
    <div class="toast-item-content" @click="onConfirm">
      <div class="toast-item-author avatar-wrapper" v-if="toast.user_id">
        <Avatar :user-id="toast.user_id" :has-avatar="user.has_avatar" :cache="user ? user.updated_on_ts : 0" size="small" />
      </div>
    ...
    <button type="button" v-if="toast.close" class="btn btn-icon btn-sm" @click="onDismiss(toast.id)">
...
export default {
  ...
  computed: {
    channel() {
      if (this.toast.channel_id) return this.$store.state.channels.single[this.toast.channel_id];
      return this.$store.state.channels.single[this.$route.params.id];
    },
    user() {
      return this.$store.getters["channels/getUserInChannel"](this.channel.id, this.toast.user_id);
    },
  },
  components: {
    Icon,
