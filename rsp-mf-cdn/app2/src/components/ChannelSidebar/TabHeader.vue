<template>
  <div class="sidebar-tab-header">
    <div class="first-line" v-if="user.id">
      <div class="filters">
        <button class="btn" @click="emitter('onFullView')" v-if="isActionsContext">
          <Icon family="far" name="expand" />
          <span>{{ $t("actions.view-board") }}</span>
        </button>

        <fragment v-if="isFilesContext">
          <button class="btn btn-primary" @click="emitter('onUpload')" :disabled="value.type === 'links'">
            <Icon family="far" name="plus" />
            <span>{{ $t("files.upload") }}</span>
          </button>
          <button class="btn expand" @click="emitter('onFullView')" :disabled="value.type !== ''">
            <Icon family="far" name="expand" />
    ...
    <div class="second-line">
      <div class="form-group row">
        <input
          ...
          type="text"
          :placeholder="isActionsContext ? $t('actions.search-actions') : $t('files.search-files')"
          aria-label="Search"
      ...
      <div class="filters">
        ...
        >
          <div class="has-mini-icon">
            ...
            <Icon family="fas" name="caret-up" v-if="value.sort == 6" class="mini" />
            <Icon family="fas" name="caret-down" v-if="value.sort == 5" class="mini" />
          </div>
        </button>
        <button
          v-if="isActionsContext"
        ...
        <Popover popover-class="files-filters-settings" v-if="isFilesContext">
          <button type="button" class="btn btn-icon btn-sm" :disabled="value.type === 'links'">
          ...
          <template slot="popover">
            <div class="dropdown-items">
              ...
              <button
                ...
                "
              >
                <Icon family="fal" name="check" :class="{ active: value.sort_by === 'updated_on' }" />{{ $t("files.filters.sort-by-activity") }}
              </button>
              <button
                v-close-popover
                class="dropdown-item"
                ...
                "
              >
                <Icon family="fal" name="check" :class="{ active: value.sort_by === 'name' }" />{{ $t("files.filters.sort-by-name") }}
              </button>
              ...
              <fragment v-if="value.type === ''">
                ...
                <button
                  ...
                  "
                >
                  <Icon family="fal" name="check" :class="{ active: value.gridView }" />{{ $t("files.filters.grid-view") }}
                </button>
                <button
                  v-close-popover
                  class="dropdown-item"
                  ...
                  "
                >
                  <Icon family="fal" name="check" :class="{ active: !value.gridView }" />{{ $t("files.filters.list-view") }}
                </button>
              </fragment>
            </div>
          </template>
...
export default {
  ...
  computed: {
    ...
    actions() {
      return this.$store.getters["actions/getActionsByChannel"](this.channel.id);
    },
