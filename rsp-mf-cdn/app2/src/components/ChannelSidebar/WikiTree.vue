<template>
  ...
  >
    ...
    <p v-if="tree.summary && tree.summary[1]" class="item-content text-truncate">
  ...
  >
    ...
    <ul>
      <WikiTree v-for="(item, index) in tree.items" :key="index" :tree="item" :current="current" :channel="channel" />
    </ul>
  </div>
</template>
