  ...
  <div v-if="active && channel" class="card card-transparent">
    <div class="card-header card-header--has-border">
      <h2 class="mb-0">
        ...
        />
        <div v-if="currentWikiPage !== 'index.md'" class="wiki-nav d-flex align-items-center">
          <router-link
    ...
    <div class="collapse d-flex flex-column show">
      <div class="card-body">
        ...
        <perfect-scrollbar
          ...
          class="wiki-content"
          @click="onClickWiki"
          v-html="wiki && wiki[currentWikiPage] ? wiki[currentWikiPage].html : ''"
        ></perfect-scrollbar>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch: {
    ...
    channel(next, previous) {
      if (((next && previous && next.id !== previous.id) || !previous) && this.active) {
        this.load();
      }
    },
  },
  ...
  methods: {
    ...
    async onClickWiki(e) {
      if (!this.isElectron) {
        ...
        if (e.target.hostname && e.target.hostname === window.location.hostname) {
          e.preventDefault();
          e.stopPropagation();
