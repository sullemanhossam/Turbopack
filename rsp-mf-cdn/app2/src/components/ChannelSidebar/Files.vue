<template>
  <div v-if="active" class="sidebar-tab sidebar-tab-files">
    <TabHeader @onUpload="onClickUpload" context="files" :value="filters" @input="onUpdateFilters" @onFullView="openExplorerModal" />
    <div class="sidebar-tab-body">
      <div class="tabbed--wrapper">
        <ul class="nav nav-pills nav-fill nav-justified">
          <li class="nav-item">
            <a class="nav-link py-3" :class="{ active: currentTab === 'media' }" @click="switchTab('media')">{{ $t("files.tabs.media") }}</a>
          </li>
          <li class="nav-item">
            <a class="nav-link py-3" :class="{ active: currentTab === 'files' }" @click="switchTab('files')">{{ $t("files.tabs.files") }}</a>
          </li>
          <li class="nav-item">
            <a class="nav-link py-3" :class="{ active: currentTab === 'links' }" @click="switchTab('links')">{{ $t("files.tabs.links") }}</a>
          </li>
        </ul>
        <div class="tabbed--content tabbed--content-media" v-if="currentTab === 'media'">
          <FileExplorer
            context="media"
            key="media"
        ...
        <div class="tabbed--content tabbed--content-files" v-if="currentTab === 'files'">
          <FileExplorer
        ...
        <div class="tabbed--content tabbed--content-links" v-if="currentTab === 'links'">
...
export default {
  ...
  methods: {
    ...
    updateQueryFilters() {
      const queryFilters = this.parseQueryFilter(this.filters, this.currentPath, this.channelPath);
    ...
    loadFiltersFromQuery() {
      ...
      if (!_.isEqual(this.$route.query, queryFilters)) {
        if ((this.currentPath && this.filters.type === "media") || this.filters.type === "") {
          if (!this.isLoadingPath(this.currentPath, this.filters.type)) {
            this.$store.dispatch("files/get", { path: this.currentPath });
          }
        } else if (this.filters.type === "links") this.$store.dispatch("files/getLinks", this.channel.id || this.$route.params.id);
      }
    },
    openExplorerModal() {
