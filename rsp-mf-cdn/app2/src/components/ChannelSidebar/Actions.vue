<template>
  <div v-if="active" class="sidebar-tab sidebar-tab-actions">
    <div>
      <TabHeader @onFullView="openFullView" context="actions" :value="fullActionFilters" @input="onUpdateFilters" />
      <div class="sidebar-tab-body">
        <AddAction ref="addActionComponent" :channel_id="channel.id" v-if="user.role.can('action.create')" :tooltip="$t('actions.add-task-tip')" />
        ...
        <div v-if="isActionsEmpty && !loading" class="empty-wrapper">
          <div class="empty-box">
            ...
            <div class="row">
              <button v-if="contextActions.length" @click="showAllActions" class="btn btn-outline-primary mx-2">
                {{ $t("actions.show-all") }}
              </button>
    ...
    <ActionFullViewModal @closed="isFullViewOpened = false" @opened="isFullViewOpened = true" />
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    contextActions: {
      get() {
        ...
        if (!this.channel) actions = [];
          ...
          actions = this.$store.getters["actions/getActionsByChannel"](this.channel.id);
        }
    ...
    allowUserSort() {
      return this.user.id != undefined && this.fullActionFilters.sort == 3 && this.user.role.can("action.update");
    },
    loading() {
