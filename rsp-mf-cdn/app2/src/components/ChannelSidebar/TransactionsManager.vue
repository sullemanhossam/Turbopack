      ...
      <div class="transactions">
        ...
        <div class="subheader" v-else-if="transactions.length">
          <span class="items" v-if="runningTransactions.length > 0">{{
            $tc("files.transactions-processing-items", runningTransactions.length)
          }}</span>
          <div
            class="clear-all"
            ...
            v-tooltip="{
              ...
              content: `<div class='text-left'>
                        <b>${$t('files.tooltips.transactions-clear-transactions')}</b>
                        <div class='text-left'>${$t('files.tooltips.transactions-clear-transactions-description')}</div>
                      </div>`,
            }"
          >
        ...
        <div class="transactions-list" v-if="transactions.length">
          <div class="transaction-single" v-for="(transaction, id) in transactions" :key="id">
            <FileIcon :name="transaction.name" :isFolder="transaction.file_type === 'folder'">
              <template v-slot:status>
                <Icon
                  ...
                  v-tooltip="{
                    content: $t('files.tooltips.transactions-operation-badge-processing'),
                  }"
                />
                <Icon family="fas" name="check" class="success" v-else-if="transaction.status === 'completed'" />
                <Icon
                  family="fas"
                  name="times"
                  class="error"
                  ...
                  v-tooltip="{
                    content: $t('files.tooltips.transactions-operation-badge-failed'),
                  }"
                />
                <Icon
                  family="fas"
                  ...
                  v-tooltip="{
                    content: $t('files.tooltips.transactions-operation-badge-interrupted'),
                  }"
                />
              </template>
            </FileIcon>
            <div class="info-section">
              <div class="transaction-header d-flex">
                <div class="file-name">
                  <span @click="openFile(transaction)">{{ transaction.name }}</span>
                </div>
                <div class="transaction-actions">
                  <span class="file-size" v-if="transaction.size >= 0 && (transaction.status == 'completed' || transaction.type === 'download-zip')">
                    {{ bytesToSize(transaction.size) }}
                  </span>
                  <span class="file-size" v-else-if="transaction.status == 'processing' && transaction.progress >= 0 && transaction.progress <= 100">
                    {{ parseInt(transaction.progress) }}%
                  </span>
                  <Popover v-if="user && user.id && transaction.status == 'completed'" placement="left">
                    <button
                      v-tooltip="{
                        delay: 1000,
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        <button
                          ...
                          v-if="transaction.status == 'completed' && transaction.downloadPath"
                        >
                          <Icon family="fal" name="folder" />
                          {{ $t("files.more-options.transactions-operation-action-show-in-download-folder") }}
                        </button>
                        ...
                        >
                          <Icon family="fal" name="folder" />
                          {{ $t("files.more-options.transactions-operation-action-show-in-airsend-folder") }}
                        </button>
                        ...
                        >
                          ...
                          {{ $t("files.more-options.transactions-operation-action-show-in-airsend") }}
                        </button>
                        ...
                        >
                          <Icon family="fal" name="file" />
                          {{ $t("files.more-options.transactions-operation-action-show-in-machine") }}
                        </button>
                      </div>
                    </template>
                  </Popover>
                  <span
                    ...
                    v-else-if="transaction.status == 'processing' || transaction.status == 'preparing'"
                    v-tooltip="{
                      content: $t('files.tooltips.transactions-operation-action-cancel'),
                      offset: '10px',
                      placement: 'left',
                    }"
                  >
              ...
              <Progress v-if="transaction.status == 'processing'" :value="transaction.progress" />
              <p class="transaction-description" v-if="transaction.status !== 'processing'">
                {{
                  transaction.message === "string"
                    ? transaction.message
                    : transaction.message.i18n.count !== undefined
                    ? $tc(transaction.message.key, transaction.message.i18n.count)
  ...
  mounted() {
    if (window.ipcRenderer) {
      ipcRenderer.on("download-completed", (e, name, downloadPath) => {
        let transaction = this.$store.getters["files/getTransactionByFileName"](name);
        if (transaction) {
          this.setTransaction({
            id: transaction.id,
            downloadPath,
  ...
  methods: {
    ...
    cancelTransaction(transaction) {
      this.setTransaction({
        ...
        status: "interrupted",
        message: {
          key: transaction.type === "upload" ? "files.transactions-messages.upload-interrupted" : "files.transactions-messages.download-interrupted",
    ...
    openFileFolder(transaction) {
      ...
      this.$router.push(`/channel/${channel_id}/files/${finalPath.join("/")}${`?${qs.stringify(query)}`}`);
    },
