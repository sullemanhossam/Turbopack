  ...
  <div>
    <div class="form-group" :class="{ [`is-invalid`]: errors['emails'] }">
      <label for="emails" class="mb-2 optional"
        >{{ $t(teamContext ? "teams.forms.add-people-to-the-team" : "channels.forms.add-people-to-the-channel") }}
        <span class="optional" v-if="!teamContext">{{ $t("general.optional") }}</span></label
      >

      <AutoComplete
        ref="emailBox"
        :data="siblings"
        :placeholder="$t('channels.forms.add-people-to-the-channel-placeholder')"
        @select="onSelectAssignee"
        @noSelectionKeyup="onChangeEmails"
      />
      <small v-if="errors['emails']" class="form-text text-danger">{{ errors["emails"] }}</small>
    </div>
    ...
    <div v-if="this.emails.length > 0" class="form-group chip-group mb-0">
      <div v-for="(user, index) in this.emails" :key="index" class="chip">
        ...
        <button type="button" name="button" class="btn btn-icon" @click="onRemoveEmail(index)">
...
export default {
  ...
  methods: {
    ...
    onChangeEmails: function (e, v) {
      ...
      if ((!e || separatorKeys.indexOf(e.key) > -1 || e.key === "Enter" || e.type === "blur") && emailList !== "") {
        if (e) {
          e.stopPropagation();
          e.preventDefault();
        }
        ...
        for (var i = 0; i < emailList.length; i++) {
          ...
          if (Utils.validateEmail(email)) {
            ...
            if (this.emails.indexOf(email) === -1 && _.findIndex(this.emails, { email }) === -1) {
              // push simple email
              this.emails.push(email);

              // check if user exists in system
              this.$store.dispatch("channels/checkUser", email).then((response) => {
                if (response) {
        ...
        if (invalidEmails.length === 0) {
          ...
          Vue.set(this.errors, "emails", this.$t("general.errors.invalid-email"));
        }
      }
      this.$emit("updateEmails", this.emails);
      return e;
