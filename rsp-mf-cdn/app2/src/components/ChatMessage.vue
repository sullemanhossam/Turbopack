<template>
  ...
  >
    <MemberCard :author="author" :message="message" :channel="channel" :team_id="channel.team_id" />
...
export default {
  ...
  computed: {
    ...
    // message author
    author() {
      const member = this.$store.getters["channels/getUserInChannel"](this.channel.id, this.message.user_id);
  ...
  methods: {
    visibilityChanged(isVisible, entry, message) {
      ...
      if (this.channel.read_watermark_id) {
        ...
        // when a message is visible
        if (isVisible && message.user_id !== this.user.id && userIndex === -1 && !this.IS_READONLY) {
          if (document.hasFocus()) {
            ...
            setTimeout(this.visibilityChanged.bind(this, true, null, message), 3000);
          }
        }
      }
    },
