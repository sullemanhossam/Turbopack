    ...
    <div class="action-container col-12 p-0 fullheight">
        ...
        <div class="first-line">
          <div class="first-line-content">
            <div class="form-group">
              ...
              <select name="users-filter" class="form-control" v-model="fullActionFilters.action_status">
            ...
            <div class="form-group">
              ...
              <select name="channels-filter" class="form-control" v-model="fullActionFilters.channel_id" ref="channel_selector">
                <option :value="0">All channels</option>
                <option :value="channel.id" v-for="channel in channels" :key="`option-channel-${channel.id}`">
                  {{ channel.channel_name }}
                </option>
              </select>
            </div>
            <div class="form-group">
              <label for="sort-actions-by">{{ $t("channels.filters.sort-by") }}</label>
              <select name="sort-actions-by" class="form-control" v-model="fullActionFilters.sort">
                <option :value="3">{{ $t("general.channels") }}</option>
                <option :value="4">{{ $t("general.channels") }} - {{ $t("actions.descending") }}</option>
                <option :value="1">{{ $t("general.name") }}</option>
                <option :value="2">{{ $t("general.name") }} - {{ $t("actions.descending") }}</option>
                <option :value="5">{{ $t("general.due-date") }}</option>
                <option :value="6">{{ $t("general.due-date") }} - {{ $t("actions.descending") }}</option>
              </select>
            </div>
          </div>
        </div>
        <div class="second-line">
          <AddAction ref="addActionComponent" :channel_id="fullActionFilters.channel_id" :tooltip="$t('actions.add-task-tip')" />

          <div class="mode-switch">
            <button class="btn btn-icon" @click="setContext('table')" :class="{ active: context === 'table' }">
      ...
      <div v-if="isActionsEmpty" class="empty-wrapper">
        <div class="empty-box">
          ...
          <div class="row">
            <button @click="addActionFocus" class="btn btn-primary mx-2" v-if="user.role.can('action.create')">
              {{ $t("actions.add-action") }}
            </button>
          </div>
        </div>
      </div>
      <div v-else-if="context === 'grid'" class="full-action-view-actions-body-grid-wrapper">
        <div class="full-action-view-actions-body full-action-view-actions-body-grid">
  ...
  computed: {
    isActionsEmpty() {
      ...
      if (this.context === "table") {
        ...
        return (
          this.$store.getters["actions/getActionsByChannel"](this.fullActionFilters.channel_id) &&
          this.$store.getters["actions/getActionsByChannel"](this.fullActionFilters.channel_id).length === 0
        );
      }
    },
    fullActionFilters: {
    ...
    user() {
      return this.$store.getters["core/getUser"](this.fullActionFilters.channel_id);
    },
  },
  methods: {
    async load() {
