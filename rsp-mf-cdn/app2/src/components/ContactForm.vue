    ...
    <div class="form-group form-section">
      <div class="row">
        ...
        <div class="col-md-3" v-if="!(contactForm && contactForm.id)">
          <button class="btn btn-link mt-md-3" type="button" @click="createForm">
            <Icon name="plus" />
        ...
        <div class="col-md-12" v-else>
          ...
          <div class="row">
            <div class="col-md-9">
              <div class="code-wrapper" id="script-container" @click="copyScript">
                <div class="copy-control">
                  {{ copied ? $t("settings.contact-form.code-copied") : $t("settings.contact-form.code-copy") }}
                </div>
                <code>
                  &lt;div class="airsend-contact-src" data-airsend-form-hash="{{ contactForm.form_hash }}" data-airsend-form-color="{{ themeColor }}"
            ...
            <div class="col-md-3">
              <button class="btn btn-link mx-sm-2 mt-md-3" type="button" @click="copyScript">
                <Icon name="copy" />
                {{ $t("settings.contact-form.button-copy") }}
              </button>
              <button class="btn btn-link mx-sm-2" type="button" @click="deleteForm">
                <Icon name="trash-alt" />
            ...
            <div class="col-md-9">
              ...
              <div class="form-group text-center" v-if="responseMessage !== null">
                <textarea class="form-control" v-model="responseMessage"></textarea>
              </div>
            </div>
            <div class="col-md-3">
              <button class="btn btn-link mx-sm-2 mt-md-3" type="button" @click="saveForm"><Icon name="copy" /> {{ $t("general.save") }}</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      themeColor: "#0097C0",
      linkUrl:
        window.origin.includes("live.airsend.io") || window.origin.includes("file://")
  ...
  methods: {
    updateForm() {
      ...
      if (this.contactForm.color) this.themeColor = "#" + this.contactForm.color;
    },
    copyScript() {
