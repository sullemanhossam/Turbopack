  ...
  <form class="form-inline my-2 my-lg-0 mr-2 search" @submit="onSubmit">
    <div class="form-group form-group--icon">
      ...
      <input
        ...
        type="text"
        :placeholder="channel ? $t('navbar.search-in-channel') : $t('navbar.search-channels-and-files')"
        aria-label="Search"
      />
      <button
        type="button"
    ...
    <div
      ...
      v-if="isOpen"
      v-bind:class="{
        [`is-loading`]: isSearching || (!isSearching && Object.keys(results).length === 0),
      }"
    >
      <Loader :loading="isSearching" full />

      <perfect-scrollbar v-click-outside="onClickOutside" :options="{ wheelPropagation: false }" v-if="!isSearching">
        <div class="empty-wrapper" v-if="!isSearching && Object.keys(results).length === 0">
          <div class="empty-box">
        ...
        <section v-if="results.channels && results.channels.length > 0">
          <div class="section-header">
            ...
            <button class="btn btn-sm btn-ghost" @click="openModal('search-view', { tab: 'channels', query })">
              {{ $t("general.show-all") }}
            </button>
          </div>
            ...
            <div class="list-group popover-list popover-list--auto">
              ...
              >
                <div class="w-100">
                  ...
                  <p v-for="(highlight, index) in item.highlighted" :key="index">
                    <small class="search-result text-muted" v-html="highlight"></small>
                  </p>
        ...
        <section v-if="results.messages && results.messages.length > 0">
          <div class="section-header">
            ...
            <button class="btn btn-sm btn-ghost" @click="openModal('search-view', { tab: 'messages', query })">
              {{ $t("general.show-all") }}
            </button>
          </div>
          <div class="section-body">
            <div class="list-group popover-list popover-list--auto">
              <router-link
                ...
                :to="{
                  name: $route.name == 'channel-sub' ? 'channel-sub' : 'channel',
                  params: {
                    ...$route.params,
                  ...
                  <p class="mb-0">
                    {{
                      $t("messages.search-title", {
                        ...
                        channelName: getChannel(item.message.channel_id).channel_name,
                      })
                    }}
                  </p>
                  <p v-for="(highlight, index) in item.highlighted" :key="index">
        ...
        <section v-if="results.actions && results.actions.length > 0">
          <div class="section-header">
            ...
            <button class="btn btn-sm btn-ghost" @click="openModal('search-view', { tab: 'actions', query })">
              {{ $t("general.show-all") }}
            </button>
          </div>
          <div class="section-body">
        ...
        <section v-if="results.files && results.files.length > 0">
          <div class="section-header">
            ...
            <button class="btn btn-sm btn-ghost" @click="openModal('search-view', { tab: 'files', query })">
              {{ $t("general.show-all") }}
            </button>
          </div>
          <div class="section-body">
              ...
              >
                <div class="w-100">
                  <div class="d-flex justify-content-between">
                    ...
                    <div v-if="item.file.is_wiki_file" class="member-badge badge">WIKI</div>
                  </div>
                  <small class="text-muted"
        ...
        <section v-if="results.users && results.users.length > 0">
          <div class="section-header">
            ...
            <button class="btn btn-sm btn-ghost" @click="openModal('search-view', { tab: 'users', query })">
              {{ $t("general.show-all") }}
            </button>
          </div>
          <div class="section-body">
    ...
    onSearch: function (e) {
      ...
      if (value !== "") {
        ...
        this.timeout = setTimeout(async () => {
          ...
          await this.$store.dispatch("core/search", !this.channel ? { query: value } : { query: value, channel: this.channel.id });
          Vue.set(this, "searching", this.searching - 1);
        }, 250);
      } else {
