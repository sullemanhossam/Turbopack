<template>
  <Drop v-cloak class="chat-fragment" :class="{ [`is-dragging`]: isDragActive }" @drop="onDropFile" @dragenter="onDragEnter" @dragleave="onDragLeave">
      ...
      if (e.dataTransfer.items.length > 0 || ext) {
        ...
        if (e.dataTransfer.items.length > 0) {
          ...
          for (let i = 0; i < droppedFiles.length; i++) {
            const entry = droppedFiles[i] instanceof DataTransferItem ? droppedFiles[i].webkitGetAsEntry() : droppedFiles[i];
