<template>
  <portal to="modal-wrapper">
    <Modal name="file-picker" :title="$t('files.modals.pick-file-title')" class="picker-modal" @before-open="onOpen">
      <form novalidate="true" @submit="onSubmit">
        <ol v-if="breadcrumb" class="breadcrumb breadcrumb--light">
          <li v-for="(route, index) in breadcrumb" :key="route.path" class="breadcrumb-item">
            <a v-if="index !== breadcrumb.length - 1" @click="browse(route.path)">{{ route.title }}</a>
        ...
        <button type="submit" class="btn btn-primary btn-rounded mx-auto d-block mt-4" :disabled="!target || target === ''">
