<template>
  <Modal name="transfer-channel-ownership" :title="$t('channels.settings.channel-transfer-ownership')" @before-open="onOpen">
    <Loader :loading="this.$store.state.loading['channels/transfer']" full />
    <div class="transfer-channel-ownership">
      <span v-if="this.error" class="alert alert-danger d-block mb-3">{{ this.error }}</span>
      ...
      />
      <ul class="my-3">
        <li v-html="$t('channels.settings.channel-transfer-ownership-single-owner-information')" />
        <li v-html="$t('channels.settings.channel-transfer-ownership-owner-information', { new_owner: new_owner && new_owner.display_name })" />

        <li
          v-if="team && isNewOwnerPartOfTeam"
          v-html="
            $t('channels.settings.channel-transfer-ownership-team-information', {
              channel_name: channel && channel.channel_name,
              team_name: team.name,
            })
          "
        ...
        <li
          v-if="team && !isNewOwnerPartOfTeam"
          v-html="
            $t('channels.settings.channel-transfer-ownership-owner-out-of-team-information', {
              new_owner: new_owner && new_owner.display_name,
              team_name: team.name,
            })
          "
      ...
      <span
        class="my-3"
        v-html="
          $t('channels.settings.channel-transfer-ownership-confirmation-message', {
            channel_name: channel && channel.channel_name,
            new_owner: new_owner && new_owner.display_name,
          })
        "
      ...
      <input type="text" class="form-control my-3" v-model="ownerNameInput" :placeholder="new_owner && new_owner.display_name" />

      <button class="btn btn-primary" :disabled="ownerNameInput !== (new_owner && new_owner.display_name)" @click="onSubmit">Save</button>
    </div>
  </Modal>
</template>
<script>
...
export default {
  ...
  methods: {
    onOpen(e) {
      ...
      if (channel && channel.team_id && !this.$store.state.teams.members[channel.team_id]) {
    ...
    async onSubmit(e) {
      ...
      if (this.team_transfer) {
        response = await this.$store.dispatch("teams/transferChannelOwnership", payload);
      } else {
