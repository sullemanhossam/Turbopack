<template>
  <portal to="modal-wrapper" :order="2">
    <Modal name="channel-create" :title="$t('general.create-new-channel')" @opened="onOpen">
      ...
      <form novalidate="true" @submit.prevent.stop="onSubmit">
        ...
        <div class="form-group" :class="{ [`is-invalid`]: errors['channel_name'] }">
          ...
          <input id="channel_name" ref="channel_name" v-model="form.channel_name" type="channel_name" class="form-control" placeholder autofocus />
        ...
        <div class="p-3 mb-2 boxed boxed--light">
          <div class="custom-control custom-switch">
            <input id="is_public" v-model="form.is_public" type="checkbox" class="custom-control-input" />
        ...
        <div class="form-group mt-0" :class="{ [`is-invalid`]: errors['copy_from_channel_id'] }">
          ...
          <select
            ...
            autofocus
          >
            <option value="" selected>{{ $t("channels.forms.optionally-select-a-template") }}</option>
            <option v-for="channel in actualChannels" v-if="currentUser.id === channel.created_by" :value="channel.id">
              {{ channel.channel_name }}
            </option>
          </select>
          <small v-if="errors['copy_from_channel_id']" class="form-text text-danger">{{ errors["copy_from_channel_id"] }}</small>
        </div>

        <hr class="my-4" />

        <button class="btn btn-primary btn-rounded mx-auto d-block" :class="{ disabled: this.form.channel_name === '' }">
