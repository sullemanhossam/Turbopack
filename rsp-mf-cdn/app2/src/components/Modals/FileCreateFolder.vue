<template>
  <portal to="modal-wrapper" :order="2">
    <Modal name="file-create-folder" :title="$t('files.modals.create-folder-title')" @before-open="beforeOpen" @opened="onOpen">
      ...
      <form novalidate="true" @submit="onSubmit">
        ...
        <div class="form-group" :class="{ [`is-invalid`]: errors['fsname'] }">
          <label for="fsname" class="mb-2">{{ $t("files.modals.create-folder-form") }}</label>
          <input id="fsname" ref="fsname" v-model="form.fsname" type="fsname" class="form-control" placeholder="" autofocus />
          <small v-if="errors['fsname']" class="form-text text-danger">{{ errors["fsname"] }}</small>
        </div>
