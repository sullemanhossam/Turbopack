<template>
  <Modal name="incoming-meeting-invite" className="modal-meeting" :title="$t('meeting.invite.title')" @closed="onClose">
    ...
    <form novalidate="true" @submit="onSubmit" v-if="channel && invitedBy">
      ...
      <div class="modal-meeting-info text-center">
        ...
        />
        <Avatar :name="channel.channel_name" :channel-id="channel.id" type="logo" :cache="0" :has-avatar="channel.has_logo" size="full" v-else />
...
export default {
  ...
  computed: {
    ...
    channel() {
      ...
      return this.$store.getters["channels/getChannelById"](this.invite.channel_id);
    },
    invitedBy() {
    ...
    callHeading() {
      return this.channel.counterpart ? this.channel.counterpart.display_name : this.channel.channel_name;
    },
    callBody() {
  ...
  watch: {
    invite(newVal, oldVal) {
      if (newVal && (!oldVal || !newVal || newVal.call_hash !== oldVal.call_hash)) {
        this.open();
        if (window.isElectron) {
