<template>
  <div>
    <Modal @opened="onOpen" name="rename-group" :title="$t('channels.groups.rename-group')">
      ...
      <form novalidate="true" @submit.prevent.stop="onSubmit">
        ...
        <div class="form-group text-center mt-5">
          <button class="btn btn-primary btn-rounded mx-2" :class="{ disabled: this.inpModel === '' }">
            {{ $t("channels.groups.rename-group") }}
          </button>
          <button class="btn btn-primary btn-ghost mx-2" type="button" @click="$modal.hide('rename-group')">
            {{ $t("general.cancel") }}
          </button>
        </div>
      </form>
