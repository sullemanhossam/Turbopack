<template>
  <Modal name="team-create" title="Create New Team" theme="noPadding" className="new-team" @before-open="beforeOpen" @opened="onOpen">
    ...
    <div v-if="page === 'team-creation'" class="team-creation">
      ...
      <form novalidate="true" @submit.prevent.stop="onSubmit" class="px-5 py-4">
        ...
        <div class="form-group" :class="{ [`is-invalid`]: errors['name'] }">
          <label for="team_name" class="mb-2">{{ $t("teams.team-name") }}</label>
          <input id="team_name" ref="team_name" v-model="form.name" type="team_name" class="form-control" placeholder autofocus />
        ...
        <div class="form-group text-center">
          <button class="btn btn-primary btn-rounded mx-2" :class="{ disabled: this.form.name === '' }">
            {{ $t("teams.forms.create-team") }}
          </button>

          <button class="btn btn-primary btn-ghost mx-2" type="button" @click="$modal.hide('team-create')">
            {{ $t("general.cancel") }}
          </button>
        </div>
      </form>
    ...
    <div class="team-billing" v-if="page === 'team-billing'">
      <div class="team-billing-header">
        ...
        <ul>
          ...
          <li>$ 4.99 per user, per month, minimum 5 users after the test period</li>
        </ul>
        ...
        <div class="seats-slider">
          ...
          <Slider v-model="seats" :teamSeatsUsed="teamSeatsUsed" :teamSeatsAvailable="teamSeatsAvailable" />
        </div>

        <div class="seats-legend" v-if="teamSeatsUsed || teamSeatsAvailable">
          <div class="legend-item taken-section">Taken seats ({{ teamSeatsUsed }})</div>
          <div class="legend-item unused-seats-section">Unused seats ({{ teamSeatsUnused }})</div>
          <div class="legend-item new-seats-section">New seats ({{ teamSeatsNew }})</div>
        </div>

        <div class="subscription-types">
          <div class="subscription-option" :class="{ active: activePlan === 'montly' }" @click="activePlan = 'montly'">
          ...
          <div class="subscription-option" :class="{ active: activePlan === 'yearly' }" @click="activePlan = 'yearly'">
          ...
          <small class="subscription-info-text">* This billing includes only new seats. </small>
        </div>
            ...
            <Popover>
              ...
              <template slot="popover">
                <div class="dropdown-items">
                  <button v-close-popover class="dropdown-item" type="button" @click="activePaymentMethod = 'New credit card'">
                    Add credit card
                  </button>
                  <button
                    v-close-popover
        ...
        <button class="btn btn-primary mx-auto d-block" @click="onCreateTeam">Start free trial</button>
      </div>
    </div>
  </Modal>
</template>
...
import Slider from "@/components/Teams/AddSeatsSlider.vue";
...
import TeamReadMore from "@/components/Banners/TeamReadMore.vue";
