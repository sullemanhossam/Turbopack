<template>
  <div>
    <Modal name="channel-add-members" :title="$t('channels.invite-title')" @opened="onOpen">
      <Loader :loading="this.$store.state.loading['channel.invite'] || this.$store.state.loading['channel.update']" full />
      <form novalidate="true" @submit.prevent.stop="onSubmitAddMembers">
        <div v-if="typeof errors === 'string'" class="alert alert-danger">
          {{ errors }}
        </div>
        <AddMembers ref="addMember" :siblings="siblings" @updateEmails="(emails) => (form.emails = emails)" />

        <div class="form-group text-center pt-2">
          <button class="btn btn-primary btn-rounded px-4 mx-2">
            {{ $t("general.add") }}
          </button>
          <button class="btn btn-primary btn-ghost mx-2" type="button" @click="$modal.hide('channel-add-members')">
            {{ $t("general.cancel") }}
          </button>
        </div>
