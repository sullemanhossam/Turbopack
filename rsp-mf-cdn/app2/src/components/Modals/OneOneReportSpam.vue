<template>
  <Modal name="one-one-report-spam" title="Report and Block" @opened="onOpen" @before-open="beforeOpen">
    <Loader :loading="isLoading" full />

    <div class="one-one-report-spam">
      <p>
        <b>{{ target.username }}</b> is going to be reported to the AirSend administration and if the spam is confirmed the user will be globally
      ...
      <div class="buttons-container">
        <button class="btn btn-primary" @click="onReportSpam">Report and Block</button>
        <button class="btn btn-outline-primary" @click="$modal.hide('one-one-report-spam')">Cancel</button>
      </div>
    </div>
  </Modal>
</template>
