<template>
  <portal to="modal-wrapper" :order="1">
    <Modal name="team-settings" :title="team && team.name" className="teams-settings" @before-open="onOpen">
      <template v-slot:tabs>
        <ul>
          ...
          <li
            ...
            v-if="user.teamRole.can('settings.tabs.announcements') && team.announcements"
            v-tooltip="{
              delay: { show: 500 },
      ...
      <template class="body" v-slot:body>
        ...
        <Members v-else-if="page === 'members'" :team="team" @inviteUser="inviteUser" />
        <Channels v-else-if="page === 'channels'" :team="team" />
        <AddMember v-else-if="page === 'invite-user'" :team="team" @return="page = 'members'" />
        <OpenChannels
...
<script>
import Modal from "@/components/Modals/templates/LeftTabbedModal.vue";
...
export default {
  ...
  computed: {
    ...
    team() {
      return this.teamId ? this.$store.getters["teams/getTeamById"](this.teamId) : {};
    },
  },
  methods: {
    onOpen(e) {
