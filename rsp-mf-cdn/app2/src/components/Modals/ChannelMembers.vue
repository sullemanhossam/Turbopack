<template>
  <div>
    <Modal name="channel-members" :title="$t('general.members')" theme="noPadding" class-name="channel-settings" @opened="onOpen">
      <Loader :loading="loading" full />

      <div class="tabbed--wrapper">
        <div class="tabbed--message" v-if="channel.pending_members && channel.pending_members.length > 0">
          {{ $tc("channels.invite.members-waiting-approval", channel.pending_members.length) }}
          <a class="ml-2" @click="currentTab = 'requires-approval'" v-if="currentTab !== 'requires-approval'">{{ $t("general.view") }}</a>
        </div>
        ...
        <ul class="nav nav-pills nav-dynamic">
          <li class="nav-item">
            <a
              ...
              v-if="meeting.channelId == channel.id || $route.name === 'meeting'"
              >{{ $t("meeting.in-the-meeting") }} ({{ meeting.peers.length + 1 }})</a
            >
          </li>
          <li class="nav-item">
            <a
              ...
              >{{ $t("general.members") }} ({{ channel.members && channel.members.length }})</a
            >
          </li>
          <li v-if="user.role.can('channel.manage') && channel.pending_members" class="nav-item">
            <a class="nav-link" :class="[currentTab === 'requires-approval' ? 'active' : '']" href="#" @click="currentTab = 'requires-approval'"
              >{{ $t("channels.invite.requires-approval") }} ({{ channel.pending_members && channel.pending_members.length }})</a
            >
          </li>
          <li v-if="(user.role.can('channel.invite') || user.teamRole.can('channel.invite')) && !channel.one_one" class="nav-item ml-auto mr-4 my-2">
            <button class="btn btn-link" type="button" @click="openNewMemberModal">
        ...
        <div class="tabbed--content">
          ...
          <perfect-scrollbar class="members-list" v-if="currentTab === 'meeting'">
            <div v-if="peers && peers.length === 0 && inviteableMembers && inviteableMembers.length === 0 && query !== ''" class="empty-wrapper">
              <div class="empty-box">
            ...
            <li class="members-list-item" v-if="query === ''">
              ...
              <div class="peer-properties">
                ...
                />
                <Icon name="video" family="far" v-tooltip="$t('meeting.member-has-webcam')" v-if="meeting.webcamProducer" />
                <Icon name="desktop" family="far" v-tooltip="$t('meeting.member-has-screen')" v-if="meeting.screenSharingProducer" />
              </div>
            </li>

            <li v-for="peer in peers" :key="peer.id" :set="(member = getUser(peer.id))" class="members-list-item">
              <Avatar
              ...
              <div class="peer-properties" :set="(consumers = getConsumers(peer.id))">
                <Icon
                  name="microphone"
                  family="far"
                  v-tooltip="$t('meeting.member-has-microphone')"
                ...
                <Popover>
                  <button
                    ...
                    v-if="meetingUser && meetingUser.id != peer.id && meetingUser.permissions.MODERATE_ROOM"
                  ...
                  <template slot="popover">
                    <div class="dropdown-items">
                      ...
                      <div
                        ...
                        v-if="(consumers.mic && !consumers.mic.remotelyPaused) || consumers.webcam || consumers.screen"
                      ></div>
                      <button v-close-popover class="dropdown-item" type="button" @click="$store.dispatch('meeting/kickPeer', peer.id)">
            ...
            <li v-for="invitee in inviteableMembers" :key="invitee.id" class="members-list-item members-list-item--transparent">
              <Avatar
              ...
              <div
                ...
                "
              ></div>
              <div v-else-if="invitee.online_status && invitee.account_status !== 50" class="members-list-item-role-lastseen">
              ...
              <div class="ml-auto d-flex">
                ...
                <div v-if="isInvited[invitee.id]" class="members-list-item-role-lastseen text-right mr-0 pr-0">
                  {{
          ...
          <perfect-scrollbar class="members-list" v-else>
            ...
            <li v-for="member in members" :key="member.id" class="members-list-item">
              ...
              <fragment v-if="ChannelRoles[member.user_role]">
                <v-popover v-if="user.role.can('channel.promote') && member.id !== user.id && member.id !== channel.owned_by">
                    ...
                    <div class="member-badge badge">
                      {{
                        channel.owned_by === member.id
                          ...
                          : $t(`channels.members-badge-${ChannelRoles[member.user_role].title}`)
                      }}
                    ...
                    <div class="dropdown-items">
                      ...
                      >
                        ...
                        <Icon
                          v-tooltip="{
                            offset: 5,
                            content: $t(`channels.members-badge-${role.title}-description`),
                          }"
                          family="fas"
                          name="info-circle"
                          class="icon-info"
                      ...
                      >
                        ...
                        <Icon
                          v-tooltip="{
                            offset: 5,
                            content: $t(`channels.members-badge-owner-description`),
                          }"
                          family="fas"
                          name="info-circle"
                          class="icon-info"
              ...
              <div
                ...
                "
              ></div>
              <div v-else-if="member.online_status && member.account_status !== 50" class="members-list-item-role-lastseen">
                {{ $t("channels.members-online-now") }}
              </div>
              <div v-else class="members-list-item-role-lastseen">
                {{ $t("channels.members-never-seen") }}
              ...
              <button
                v-if="currentTab === 'requires-approval' && user.role.can('channel.approve')"
              ...
              <!-- Approve request to join channel -->
              <v-popover v-if="currentTab === 'requires-approval' && user.role.can('channel.approve')">
                <a
                ...
                <template slot="popover">
                  <div class="dropdown-items">
                    <button v-close-popover @click="createOneToOne(member.id)" class="dropdown-item" type="button">
                      <Icon family="fal" name="paper-plane" />
                      {{ $t("channels.direct-conversation") }}
                    </button>

                    <button v-close-popover class="dropdown-item" @click="rejectJoinRequest(member.id)" type="button">
              ...
              <v-popover
                v-if="
                  currentTab === 'members' &&
                  member.id !== user.id &&
                  (user.role.can('channel.kick') || user.teamRole.can('channel.invite')) &&
                  !channel.one_one &&
                  channel.owned_by !== member.id
                "
              ...
              >
                ...
                <template slot="popover">
                  <div class="dropdown-items">
                    ...
                    <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click="removeMember(member.id)">
...
import TransferChannelOwnership from "@/components/Modals/TransferChannelOwnership.vue";
...
export default {
  ...
  computed: {
    user() {
      return this.$store.getters["core/getUser"](this.channel.id, this.team ? this.team.id : null);
    },
    meeting() {
    ...
    allMembers() {
      const allMembers = this.$store.getters["channels/getSortedMembers"](this.channel.id, this.currentTab === "requires-approval");
    ...
    teamMembers() {
      return this.$store.getters["teams/getMembers"][this.channel.team_id] || [];
    },
  },
  mounted() {
  ...
  methods: {
    onOpen() {
      this.currentTab = this.$route.name === "meeting" || (this.meeting && this.meeting.channelId == this.channel.id) ? "meeting" : "members";
      this.query = "";
      this.isInvited = {};
    ...
    removeMember: async function (memberId) {
      ...
      const response = await this.$store.dispatch("channels/removeMember", payload);
      if (response.ok) {
    ...
    async createOneToOne(id) {
      const response = await this.$store.dispatch("channels/createOneOnOne", id);
      this.$modal.hide("channel-members");
    },
    ...
    getUser(id) {
      if (this.channel.id) {
        return this.$store.getters["channels/getUserInChannel"](this.channel.id, id);
      } else {
        return null;
      }
    },
