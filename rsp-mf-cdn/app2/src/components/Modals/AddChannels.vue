<template>
  <div>
    <Modal name="add-channels" :title="$t(`channels.groups.add-channels-to-group`, { groupName: group.name })" @opened="onOpen">
      <Loader :loading="$store.state.loading['channel.group.add'] || $store.state.loading['channel.list']" full />
      ...
      <form novalidate="true" @submit.prevent.stop="onSubmit">
        ...
        <div class="row add-channels-wrapper">
          <div class="col-md-6 filter-channel">
            ...
            />
            <button v-if="filterChannel !== ''" type="button" class="btn btn-icon btn-sm cancel-search" @click="filterChannel = ''">
              <Icon family="far" name="times" class="icon-close" />
            </button>
            <div v-if="filterChannel.trim() && filteredChannels.length === 0" class="empty-wrapper">
              <div class="empty-box">
            ...
            <perfect-scrollbar v-else>
              <Drag
                v-for="channel in filterChannel.trim() ? filteredChannels : channels"
                :key="channel.id"
                :transfer-data="channel"
                :set="(counterpart = channel.one_one ? getCounterpart(channel) : null)"
                :class="[{ 'muted-channel': isChannelAdded(channel.id) }]"
                class=""
              >
                <div class="item-channel d-flex align-items-center" @click="addedChannels.push(channel)">
                  <Avatar
            ...
            <div class="droppable-area">
              ...
              <perfect-scrollbar v-else>
                ...
                >
                  ...
                  <div class="remove-from-channel" @click="removeFromChannel(channel)">
        ...
        <div class="form-group text-center mt-5">
          <button class="btn btn-primary btn-rounded mx-2" :class="{ disabled: !this.addedChannels.length }">
            {{ $t("channels.groups.add-channels") }}
          </button>

          <button class="btn btn-primary btn-ghost mx-2" type="button" @click="$modal.hide('add-channels')">
            {{ $t("general.cancel") }}
          </button>
        </div>
      </form>
...
export default {
  ...
  methods: {
    ...
    onSubmit: async function () {
      ...
      const response = await this.$store.dispatch("channels/addChannelsToGroup", { channel_group_id: this.group.id, channels });
    ...
    onOpen() {
      ...
      if (this.channels.length) {
        this.addedChannels = this.channels.filter((channel) => channel.channel_group_id == this.group.id);
    ...
    isChannelAdded(id) {
      let selectedChannels = this.addedChannels.find((channel) => id == channel.id);
    ...
    removeFromChannel(selectedChannel) {
      ...
      let isAlreadyAdded = this.channelsOfThisGroup.filter((channel) => selectedChannel.id == channel.id);
      if (isAlreadyAdded.length) {
        this.removeFromGroup(selectedChannel);
      }
      // remove from temp list
      let addedChannels = this.addedChannels.filter((channel) => selectedChannel.id != channel.id);
