<template>
  <div>
    <Modal name="channel-settings" :title="$t('channels.channel-settings')" theme="noPadding" class-name="channel-settings" @opened="onOpen">
      <div class="tabbed--wrapper">
        <ul class="nav nav-pills nav-dynamic">
          <li class="nav-item">
            <a class="nav-link" :class="[currentTab === 'general' ? 'active' : '']" href="#" @click="switchTab('general')">{{
              $t("channels.settings.general")
            }}</a>
          </li>
          <li v-if="user.role.can('channel.manage')" class="nav-item">
            <a class="nav-link" :class="[currentTab === 'customization' ? 'active' : '']" href="#" @click="switchTab('customization')">{{
              $t("channels.settings.customization")
            }}</a>
          </li>
          <li v-if="user.role.can('channel.manage') || channel.one_one" class="nav-item">
            <a class="nav-link" :class="[currentTab === 'operations' ? 'active' : '']" href="#" @click="switchTab('operations')">{{
              $t("channels.settings.channel-operations")
            }}</a>
          </li>
        </ul>
        ...
        <div class="tabbed--content">
          <div v-if="currentTab === 'general'" class="tabbed--single">
            ...
            <form @submit="onSubmitUpdate" v-if="user.role.can('channel.manage') && !channel.one_one">
              <div class="form-group" :class="{ [`is-invalid`]: errors['channel_name'] }">
                <label for="channel_name" class="mb-2">{{ $t("channels.forms.channel-name") }}</label>
                <input
                  id="channel_name"
                  ref="channel_name"
              ...
              <div class="form-group" :class="{ [`is-invalid`]: errors['blurb'] }">
                <label for="blurb" class="mb-2 optional"
                  >{{ $t("channels.forms.channel-description") }} <span class="optional">{{ $t("general.optional") }}</span></label
                >
                <textarea id="blurb" ref="blurb" v-model="form.blurb" type="blurb" class="form-control" placeholder autofocus> </textarea>
                <small v-if="errors['blurb']" class="form-text text-danger">{{ errors["blurb"] }}</small>
              </div>

              <div class="form-group mb-4" :class="{ [`is-invalid`]: errors['channel_name'] }" v-if="channel.public_url">
                <label for="channel_name" class="mb-2">{{ $t("channels.settings.permissions-title") }}</label>

                <div class="form-group form-check">
                  <input class="form-check-input" type="checkbox" v-model="form.require_join_approval" id="require_join_approval" />
                ...
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" v-model="form.allow_external_read" id="allow_external_read" />
              ...
              <div class="form-group text-center">
                <button class="btn btn-primary btn-rounded px-5 mx-2" :disabled="!formChanged">
                  {{ $t("general.save") }}
                </button>
              </div>
            </form>

            <hr class="my-4" v-if="user.role.can('channel.manage') && !channel.one_one" />

            <div class="form-group form-section" v-if="user.role.can('channel.manage') && !channel.one_one">
              <div class="row">
                <div class="col">
                ...
                <div class="col-md-3">
                  <button class="btn btn-link mx-sm-2 mt-md-3" type="button" @click="onCopyLink" v-if="channel.public_url">
                    <Icon name="copy" /> {{ $t("general.link-copy") }}
                  </button>
                  <button class="btn btn-link mx-sm-2" type="button" @click="onToggleLink" v-if="!channel.public_url">
                    <Icon name="link" /> {{ $t("general.link-create") }}
                  </button>
                  <button class="btn btn-link mx-sm-2" type="button" @click="onToggleLink" v-else>
            ...
            <div class="form-group form-section">
              <div class="row">
                <div class="col">
                  ...
                  <input ref="channelEmail" class="form-control my-3" readonly="readonly" @focus="onFocusLinkInput" :value="channel.member_email" />
                  <p>{{ $t("channels.settings.channel-email-description") }}</p>
                </div>
                <div class="col-md-3">
                  <button class="btn btn-link mx-sm-2 mt-md-3" type="button" @click="onCopyEmail">
                    <Icon name="copy" />
          ...
          <div v-if="currentTab === 'customization'" class="tabbed--single customization-tab">
            <Loader :loading="imageUploading" full />
            <form class="upload-image-placeholder" novalidate="true" @submit.prevent.stop="onImageUpdate('background')">
              ...
              <button class="btn btn-primary btn-rounded btn-full" :disabled="!background_changed">
                <Icon family="fas" name="upload" />
                {{ $t("channels.settings.channel-background-change") }}
              </button>
            </form>
            <form class="upload-image-placeholder" novalidate="true" @submit.prevent.stop="onImageUpdate('logo')">
              ...
              <button class="btn btn-primary btn-rounded btn-full" :disabled="!logo_changed">
                <Icon family="fas" name="upload" />
          ...
          <div v-if="currentTab === 'operations'" class="tabbed--single">
            ...
            <div v-if="user.role.can('channel.manage') || channel.one_one" class="form-group form-section">
              <div class="row">
                <div class="col">
                ...
                <div class="col-md-3">
                  <button class="btn btn-link mx-sm-2 mt-md-3" type="button" @click="onExportChannel">
            ...
            <div class="form-group form-section" v-if="isChannelClosed && user.role.can('channel.manage')">
              <div class="row">
                <div class="col">
                  ...
                  <v-popover>
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        ...
                        <button v-close-popover class="dropdown-item btn btn-primary" type="button" @click="onReopenChannel">
                          {{ $t("channels.settings.channel-reopen-button-confirm") }}
                        </button>
                      </div>
                    </template>
                  </v-popover>
            ...
            <div class="form-group form-section" v-else-if="user.role.can('channel.manage')">
              <div class="row">
                <div class="col">
                ...
                <div class="col-md-3">
                  <v-popover>
                    <button class="btn btn-link mx-sm-2 mt-md-3" type="button"><Icon name="lock" /> {{ $t("general.close-channel") }}</button>
                    <template slot="popover">
                      <div class="dropdown-items">
                        <div class="dropdown-text">
                          {{ $t("general.are-you-sure") }}
                        </div>
                        <button v-close-popover class="dropdown-item btn btn-primary" type="button" @click="onCloseChannel">
                          {{ $t("general.close-now") }}
                        </button>
                      </div>
                    </template>
            ...
            <hr class="my-4" v-if="user.id === channel.created_by && useTeams && !channel.one_one" />

            <div class="form-group form-section" v-if="user.id === channel.created_by && useTeams && !channel.one_one">
              <div class="row">
                <div class="col">
                ...
                <div class="col-md-3">
                  <Popover>
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        <button class="dropdown-item" type="button" v-close-popover @click="onAttachTeam(null)" :disabled="!channel.team_id">
                          <Icon family="fas" name="check" :class="{ invisible: channel.team_id }" />
                          <a>None</a>
                        </button>
                        <button
                          class="dropdown-item"
                        ...
                        >
                          <Icon family="fas" name="check" :class="{ invisible: channel.team_id !== team.id }" />
                          <a>{{ team.name }}</a>
                        </button>
                      </div>
                    </template>
            ...
            <div class="form-group form-section" v-if="user.id === channel.created_by">
              <div class="row">
                <div class="col">
                ...
                <div class="col-md-3">
                  <v-popover>
                    <button class="btn btn-link btn-link--danger mx-sm-2 mt-md-3" type="button">
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        ...
                        <button v-close-popover class="dropdown-item btn btn-danger" type="button" @click="onDeleteChannel">
...
import AttachTeamToChannel from "@/components/Modals/AttachTeamToChannel.vue";
