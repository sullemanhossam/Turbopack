<template>
  ...
  >
    <Loader :loading="this.$store.state.loading['file.upload'] || this.$store.state.loading['file.get']" full />
      ...
      <div class="edit-preview-wrapper d-flex">
        <div class="wiki-editor-pane editor-pane--edit" v-if="!viewOnly">
          <div v-if="!cached" class="wiki-editor-header">
            ...
            >
              ...
              <template slot="popover">
                <div class="wiki-editor-pane wiki-editor-pane--files">
                  <div v-if="breadcrumb" class="wiki-editor-header d-flex align-items-center justify-content-between px-3">
                    <div v-if="breadcrumb.length > 1" class="wiki-editor-toolbar">
                      <a
                        v-tooltip="{
                          delay: 1000,
                    ...
                    <div class="wiki-editor-toolbar ml-3">
                      <a
                        ...
                        class="btn btn-icon btn-sm"
                        @click="() => this.$refs.fileListMobile.openCreateFile('.md')"
                        ><Icon family="fas" name="file-plus"
                      /></a>
                      <a
                        v-close-popover
                        ...
                        class="btn btn-icon btn-sm"
                        @click="() => this.$refs.fileListMobile.openFileChooser()"
                        ><Icon family="fas" name="upload"
                      /></a>
                      <a
                        v-close-popover
                        ...
                        class="btn btn-icon btn-sm"
                        @click="() => this.$refs.fileListMobile.openCreateFolder()"
                        ><Icon family="fas" name="folder-plus"
                      /></a>
                    </div>
                  </div>
            ...
            <h4>
              {{ current }}
              <small v-if="changed || currentContent !== newContent" v-html="$t('wiki.editor.tip-unsaved-changed')"></small>
            </h4>
            <div class="wiki-editor-toolbar d-flex">
              <button
                class="btn btn-primary btn-sm"
                :disabled="isValidLocation || (currentContent === newContent && !changed)"
                href="#"
                @click="onPublish"
              >
                {{ $t("wiki.editor.button-publish") }}
              </button>
              <button class="btn btn-primary btn-sm ml-1" href="#" @click="viewOnly = true">
                {{ $t("general.cancel") }}
              </button>
            </div>
          </div>
          <div v-else class="wiki-editor-header">
            <h4>
              <Icon family="far" name="exclamation-triangle" class="text-warning mr-1" />
              {{ $t("wiki.editor.tip-viewing-draft") }}
            </h4>
            <div class="wiki-editor-toolbar">
              <button class="btn btn-default btn-sm" href="#" @click="onDraftAction(false)">
                {{ $t("general.discard") }}
              </button>
              <button class="btn btn-primary btn-sm ml-2" href="#" @click="onDraftAction(true)">
                {{ $t("general.continue") }}
              </button>
            </div>
          </div>
          ...
          <div class="wiki-editor-body wiki-editor-body--editor">
            <Loader :loading="this.$store.state.loading['file.download']" full />
              ...
              <ul class="navbar-nav flex-wrap mr-auto">
                ...
                <li class="nav-item">
                  <v-popover>
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        <button v-close-popover class="dropdown-item" type="button" @click="addHeading(1)">
                        ...
                        <button v-close-popover class="dropdown-item" type="button" @click="addHeading(2)">
                        ...
                        <button v-close-popover class="dropdown-item" type="button" @click="addHeading(3)">
                          <Icon family="fal" name="h2" />
                          {{
                            $t("wiki.editor.options.heading-sub", {
                              level: 3,
                        ...
                        <button v-close-popover class="dropdown-item" type="button" @click="addHeading(4)">
              ...
              <ul class="navbar-nav">
                <li class="nav-item">
                  <a
                    ...
                    class="btn btn-icon btn-sm"
                    @click="() => this.$refs.fileList.openFilePicker(this.location)"
                ...
                <li class="nav-item">
                  <v-popover>
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        <button v-close-popover class="dropdown-item" type="button" @click="() => this.$refs.fileList.openFilePicker(this.location)">
                          <Icon family="fal" name="folder-open" />
                          {{ $t("wiki.editor.options.add-link-wiki") }}
                        </button>
                        <button v-close-popover class="dropdown-item" type="button" @click="addLink()">
            ...
            <MarkdownEditor ref="editor" v-model="newContent" toolbar="" @input="onUpdate" />
          </div>
        </div>
        ...
        <div class="wiki-editor-pane editor-pane--preview">
          ...
          <div class="wiki-editor-body">
            ...
            <div v-if="newContent === '' && !this.$store.state.loading['file.download']" class="empty-wrapper">
              <div class="empty-box">
            ...
            <div class="wiki-content wiki-editor-body-preview" v-html="html"></div>
          </div>
        </div>
      </div>
    </div>
...
import WikiLinks from "@/components/ChannelSidebar/WikiLinks.vue";
...
export default {
  ...
  computed: {
    ...
    isValidLocation() {
      ...
      if (isInSubfolder) {
        ...
        const currentPath = this.current.split("/").slice(0, -1).join("/");
        files = this.$store.getters["files/getFilesInPath"](`${this.location}/${currentPath}`);
      } else {
  ...
  methods: {
    async load(file) {
      ...
      if (store.get(`wiki/${this.channel.id}/${file}`) && store.get(`wiki/${this.channel.id}/${file}`) !== data.markdown) {
    ...
    onSelect(selected) {
      ...
      if (ext === "md") {
        ...
        EventBus.$emit("file-preview", [{ file: file, path: `${this.currentPath}/${file}` }], 0);
      }
    },
