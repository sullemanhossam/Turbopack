<template>
  <div
    ...
    v-observe-visibility="{
      callback: (isVisible, entry) => visibilityChanged(isVisible, entry, message),
      throttle: 0,
      once: true,
    }"
    :data-plain-text="`[${parseTime(message.created_on).format('YYYY-MM-DD hh:mm A')}] ${message.plain_content}`"
  >
    <div class="chat-notice" v-if="channel">
          ...
          v-html="
            parseMessageContent(
              message.content.i18n.count
                ? $tc(message.content.i18n.key, message.content.i18n.count, i18nParams)
      ...
      <div class="notice-content" v-else>
        <Icon family="fas" :name="icon" /><span @click="onClickMessage" v-html="parseMessageContent(message.content.bot_message, channel)"></span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    i18nParams() {
      ...
      // parse due date
      if (params.due) {
        params.due = moment.unix(params.due).local().format("dddd, MMMM DD, hh:mm A");
      }
      ...
      if (params.file_blurb) {
        ...
        params.file_blurb = params.file_blurb.replace(path, encodeURIComponent(path));
      }
  ...
  methods: {
    visibilityChanged(isVisible, entry, message) {
      ...
      if (isVisible && message.id > this.channel.read_watermark_id && message.user_id !== this.user.id && !this.IS_READONLY) {
        if (document.hasFocus()) {
          ...
          // retry after 1s
          setTimeout(this.visibilityChanged.bind(this, true, null, message), 1000);
        }
      }
    },
    ...
    onClickMessage(e) {
      ...
      if (e.target.tagName === "A") {
        ...
        if (type === "path" && title.indexOf(".") > -1) {
          EventBus.$emit("file-preview", [{ file: title, path: `${path}/${title}` }], 0);
        }
        ...
        if (type === "meeting") {
          const { call_hash, server_address } = this.message.content.i18n.params;
      ...
      if (e.target.tagName === "SPAN" && e.target.className === "mention mention-user") {
      ...
      if (e.target.tagName === "SPAN" && e.target.className === "mention mention-action") {
        const { action_id, channel_id, action_name } = this.message.content.i18n.params;
