<template>
  <div id="app" v-hotkey="keymap">
    <VueHeadful :title="`${unread_count && unread_count > 0 ? `(${unread_count > 99 ? '99+' : unread_count}) ` : ''}${core.title}`" />
    <div class="preloader">
      <Loader title="Connecting to AirSend" full :loading="!this.$store.state.core.ready" />
    </div>
    <component :is="IS_READONLY ? 'Guest' : 'Main'" v-if="this.$store.state.core.ready"></component>
    <TeamSettings />
    <LanguageModal />
    <UserModal />
...
export default {
  ...
  computed: {
    ...
    IS_READONLY() {
      return !this.user.display_name || this.user.read_only === true || ["oauth"].indexOf(this.$router.history.current.name) > -1;
    },
  },
  methods: {
    updateIsMobile: _.throttle(function (e) {
      const isMobileNow = window.matchMedia("only screen and (max-width: 992px)").matches;
  ...
  watch: {
    $route(to) {
      ...
      if ((!isAuthenticated && to.meta.authenticated) || (!isAuthenticated && to.meta.requiresHash && !to.query.hash)) {
  ...
  async mounted() {
    ...
    this.$router.onReady(() => {
      this.$store.dispatch("core/handshake").then(() => {
        if (_.get(this.$store.state, "core.handshakeSettings.recaptchaEnabled", false)) {
          const recaptchaV3Key = _.get(this.$store.state, "core.handshakeSettings.recaptchaV3SiteKey", "");
          if (recaptchaV3Key.length > 0) {
            loadScript(`https://www.google.com/recaptcha/api.js?render=${recaptchaV3Key}`);
          }
        }
      });
