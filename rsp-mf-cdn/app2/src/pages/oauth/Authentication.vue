    ...
    <div class="col-md-5 auth-box">
      <div class="auth-box-content">
        ...
        <div v-if="isAuthenticated && isAppOpened" class="auth-success">
          ...
          <small @click="continueOnBrowser">{{ $t("auth.successfully-logged-in-continue-on-browser") }}</small>
        </div>
        ...
        <div class="profile-block" v-else-if="isAuthenticated && user">
          ...
          <button class="btn btn-primary" @click="onOpenApp" :disabled="isFailedToOpenApp">
            {{ $t("auth.open-app") }}
          </button>
          <button class="btn btn-secondary" @click="continueOnBrowser">
            {{ $t("auth.continue-on-browser") }}
          </button>
          <h6 v-if="isFailedToOpenApp" @click="onDownloadApp" class="download-app-advertisement" v-html="$t('auth.download-app-advertisement')"></h6>
          <hr />
        ...
        <div v-else-if="client" class="auth-proceed">
          <div v-if="client === 'apple'">
            <span class="auth-description">{{ $t("auth.apple-sign-in-description") }}</span>
            <button class="btn btn-primary auth-button" @click.stop.prevent="appleSignIn()">
              {{ $t("auth.continue") }}
            </button>
            <a href="#" class="h6" @click="onChooseLoginMethod">{{ $t("auth.choose-another-login-method") }}</a>
          </div>
        </div>
        ...
        <div class="text-center d-block">
          <a class="mt-4 mx-auto btn-sm btn btn-light" @click="() => this.$modal.show('language')">{{ $t("title") }}</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
import Logo from "@/assets/airsend-color.svg";
import Loader from "@/components/Loader.vue";
import Avatar from "@/components/Avatar.vue";
import Icon from "@/components/Icon.vue";
import BigIllustration from "@/assets/big-illustration.svg";
