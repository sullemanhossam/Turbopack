  ...
  <div class="page-content page-login">
    ...
    <div class="col-md-5 auth-box">
      <div class="auth-box-content">
        ...
        <form v-else-if="user.display_name" class="oauth-form" @submit="onSubmit">
          <Loader full :loading="this.$store.state.loading['oauth.server.authorize'] || this.$store.state.loading['oauth.server.approve']" />
...
export default {
  ...
  methods: {
    async checkClient() {
      ...
      const { client_id, redirect_uri, response_type, scope, state } = this.$route.query;

      const response = await this.$store.dispatch("core/getOauthAutorizationInfo", {
        client_id,
        response_type,
        redirect_uri,
        scope,
