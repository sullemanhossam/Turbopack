<template>
  <div v-hotkey="keymap" class="page-content">
    <div ref="container" class="container-fluid fullheight" :class="{ [`container--read-only`]: IS_READONLY }">
      <div class="row fullheight channel-row">
        <nav class="nav nav-pills nav-fill nav-justified nav-channel">
          <a class="nav-item nav-link" :class="{ [`active`]: currentPane === 'messages' }" @click="onChangePane('messages')">{{
            $t("general.messages")
          }}</a>
          <a class="nav-item nav-link" :class="{ [`active`]: currentPane === 'actions' }" @click="onChangePane('actions')">{{
            $t("general.actions")
          }}</a>
          <a class="nav-item nav-link" :class="{ [`active`]: currentPane === 'files' }" @click="onChangePane('files')">{{ $t("general.files") }}</a>
          <a class="nav-item nav-link" :class="{ [`active`]: currentPane === 'wiki' }" @click="onChangePane('wiki')">{{ $t("general.wiki") }}</a>
        </nav>

        <div v-if="currentPane === 'actions' && isMobile" class="fullscreen-card">
          <Actions :active="true" />
        </div>
        ...
        <ChannelSwitcher
          v-if="!(IS_READONLY && (!this.user || this.user.read_only)) && actualChannels.length > 0 && !isMobile"
          ref="switcher"
          @unblocked="load()"
        />
        ...
        <ChannelChatFragment v-if="currentPane === 'messages' && !hide_panes" :channel="channel" @drop="onDropFile">
          ...
          <div
            ...
            :style="bgUrlStyle"
          >
            <div class="full-toast" :class="{ show: !this.$store.state.core.websocket.status }">
              {{ this.$store.state.core.websocket.error }}
            </div>

            <form class="public-channel-info" @submit="onJoinChannel" v-if="channel && isJoinOnly">
              <Loader :loading="$store.state.loading['channel.join']" full />

              <div v-if="typeof joinErrors === 'string'" class="alert alert-danger mb-4">
                {{ joinErrors }}
              </div>

              <img v-if="channel.has_logo" style="border-radius: 100%" :src="logoUrl" :alt="channel.channel_name" />
              <h1
                v-html="
              ...
              <div class="d-block text-center" v-if="!joinSubmitted">
                <button v-if="user.id" tyle="submit" class="btn btn-primary btn-rounded mx-1 mt-4">
                  {{ $t(channel.require_join_approval ? "channels.public-ask-to-join-channel" : "channels.public-join-channel") }}
                </button>
                <button
                  v-else
                  type="button"
            ...
            <div class="chat-fragment-scroller-wrapper" v-else-if="!channel || !channel.chat.history || this.loading">
        ...
        <ChannelSidebar v-if="!hide_panes && !isJoinOnly && !isMobile" :channel="channel" ref="sidebar" />
      </div>
    </div>
...
import { retrieveFiles, bytesToSize, parseMentions, parseTime } from "airsend/utils";
...
export default {
  ...
  computed: {
    ...
    channels() {
      return this.$store.getters["channels/getSortedChannels"]("switcher").channels;
    },
    actualChannels() {
    ...
    IS_READONLY() {
      return (this.channel && this.$router.history.current.query.hash !== undefined) || this.user.read_only === true;
    },
    params() {
    ...
    bgUrlStyle() {
      if (this.channel && this.channel.has_background) {
        ...
        return {
          "background-image": `url(${process.env.VUE_APP_ROOT_API}/v1/channel.image.get?channel_id=${
            this.channel.id
          }&channel_asset_type=background&token=${hash ? hash : store.get("jwt")}&cache=${this.channel.updated_on_ts})`,
        };
      }
      return "";
    },
    ...
    teamMembers() {
      return this.channel.team_id ? this.$store.state.members[this.channel.team_id] : [];
    },
  },
  watch: {
    $route(newRoute, oldRoute) {
      if (oldRoute.params.id != newRoute.params.id || oldRoute.name !== "channel" || !!newRoute.query.highlight) {
        //channel changed
        if (oldRoute) this.clearData(oldRoute.params.id);
        this.load(
          oldRoute.params.id == newRoute.params.id && !(oldRoute.params.resource === "invite" && newRoute.name === "channel"),
  ...
  methods: {
    onTogglePanes(e) {
      ...
      if (target.nodeName !== "INPUT" && target.nodeName !== "TEXTAREA" && target.contentEditable !== "true") {
        if (e.key === "[") {
    ...
    onSwitch(e) {
      ...
      if (e.key === "ArrowDown") {
        nextChannel = this.channels[currentIndex + 1] ? this.channels[currentIndex + 1] : null;
      } else {
        nextChannel = this.channels[currentIndex - 1] ? this.channels[currentIndex - 1] : null;
      }
      ...
      if (nextChannel) {
        this.$router.push({
          ...
          params: {
            id: nextChannel.id,
            target: nextChannel.oldest_unread_message_id ? nextChannel.oldest_unread_message_id : nextChannel.read_watermark_id,
          },
        });
      }
    },
    async load(preventHistoryFetch = false, shouldJump = false) {
      ...
      if (this.currentMeeting && this.currentMeeting == this.$route.params.id && !preventHistoryFetch) {
        this.hide_panes = true;
      }
      ...
      if (this.channel) {
        ...
        // check if it's a join request alert
        if (this.$route.params.alert_type && this.$route.params.alert_type === 20) {
          this.$modal.show("channel-members");
        }

        // history
        if (!this.$route.query.highlight && !this.$route.params.target && !preventHistoryFetch) {
          await this.$store.dispatch("channels/history", this.$route.params.id);
        } else if (!preventHistoryFetch || shouldJump) {
          this.jumpTo({
            target: this.$route.query.highlight ? this.$route.query.highlight : this.$route.params.target,
        ...
        this.$store.dispatch("core/setTitle", this.channel.counterpart ? this.channel.counterpart.display_name : this.channel.channel_name);
