      ...
      <div class="auth-box-content">
        ...
        <div v-if="isDone">
          ...
          <div class="text-center d-block">
            <router-link to="/login" class="btn btn-primary btn-ghost btn-rounded btn-extended mx-auto p-relative mt-4">{{
        ...
        <form v-else novalidate="true" @submit="onSubmit">
          ...
          <div class="form-group" :class="{ [`is-invalid`]: errors['user'] }">
            ...
            <input
              ...
              disabled
            />
            <small v-if="errors['user']" class="form-text text-danger">{{ errors["user"] }}</small>
          </div>
          ...
          <div class="form-group" :class="{ [`is-invalid`]: errors['verify_code'] }">
            ...
            <input
              ...
              disabled
            />
            <small v-if="errors['verify_code']" class="form-text text-danger">{{ errors["verify_code"] }}</small>
          </div>
          ...
          <div class="text-center d-block">
            ...
            <router-link to="/login" class="btn btn-half btn-primary btn-ghost btn-rounded btn-extended mx-auto p-relative mt-3">{{
              $t("auth.back-to-login")
            }}</router-link>
          </div>
        </form>
...
export default {
  ...
  methods: {
    onSubmit: async function (e) {
      ...
      const response = await this.$store.dispatch("core/verifyAccount", this.form);
