<template>
  <div class="page-content page-channels" :class="{ ['no-use-teams']: !useTeams }">
    ...
    <Portal to="modal-toaster-area">
      <Toaster ref="portal" @mounted="onToasterWrapperCreated" @destroyed="onToasterWrapperDeleted" />
    </Portal>
    ...
    <TeamHome v-if="(teamId && $store.state.loading['teams/get']) || (currentTeam && currentTeam.name)" :teamId="teamId" />

    <section class="card-section">
      <div v-if="teamId && !$store.state.loading['teams/get'] && !(currentTeam && currentTeam.name)">
        <div class="empty-wrapper">
          <div class="empty-box empty-box--meeting">
            <Icon family="fal" name="users" class="mb-4" />
            <h4 class="text-center">No Team</h4>
            <p class="text-center mb-4">The team you are looking for do not exists or have been removed.</p>
            <router-link class="btn btn-primary mx-auto" to="/">Go back to home</router-link>
          </div>
        </div>
      </div>
      <div class="container-fluid" v-else>
        <SortFilter v-if="actualChannels.length" />
        <div v-if="!isDashboardActionsOn && actualChannels.length === 0 && $store.state.channels.isEmpty" class="welcome-user">
          <h2>
        ...
        <div class="row">
          <div :class="actualChannels.length === 0 && $store.state.channels.isEmpty ? 'empty-channel' : 'col-lg-3 col-md-4'">
            <div class="card card-float card-float--new" @click="$modal.show('channel-create', { team_id: teamId ? teamId : '' })">
              <div class="card-body">
          ...
          <div v-if="!channels.length && actualChannels.length" class="col-lg-3 col-md-4">
            <div class="no-channels-filter">
              {{
                $t(`home.filter-not-found-${userPreferences.listing.filterBy}${teamId ? "-team" : ""}`, { teamName: currentTeam && currentTeam.name })
              }}
            </div>
          </div>
          <div v-for="(channel, index) in channelList" v-else :key="index" class="col-lg-3 col-md-4">
            <router-link
              ...
              :to="{
                ...
                params: {
                  id: channel.id,
                  target: channel.oldest_unread_message_id ? channel.oldest_unread_message_id : channel.read_watermark_id,
                },
              }"
            >
              <div class="card-header">
                ...
                <h4>
                  ...
                  <span v-if="channel.unread_count > 0" class="badge badge-primary p-1" :class="{ 'badge-muted': channel.muted }">{{
                ...
                <ul class="card-nav" @click.stop.prevent>
                  ...
                  <v-popover popover-class="home-channel-settings" v-else>
                    ...
                    <template slot="popover">
                      <div class="dropdown-items">
                        <a v-if="!channel.muted" v-close-popover class="dropdown-item" href="#" @click.prevent="toggleChannelMute(channel)">
                          <Icon family="fal" name="volume-slash" />
                        ...
                        <v-popover v-if="channel.channel_status === 1 && ChannelRoles[channel.user_role].can('channel.manage')">
                          <a class="dropdown-item" href="#" @click.prevent><Icon family="fal" name="lock" /> {{ $t("channels.close-channel") }}</a>
                          <template slot="popover">
                            <div class="dropdown-items">
                        ...
                        <a
                          v-if="channel.channel_status === 2 && ChannelRoles[channel.user_role].can('channel.manage')"
                          v-close-popover
                          class="dropdown-item"
                          href="#"
                        ...
                        <a
                          v-if="ChannelRoles[channel.user_role].can('channel.manage')"
                          v-close-popover
                          class="dropdown-item"
                          href="#"
                          @click="onSubmitDuplicateChannel(channel)"
                          ><Icon family="fal" name="clone" /> {{ $t("channels.duplicate-channel") }}</a
                        >
                        ...
                        <v-popover v-if="user.id !== channel.created_by">
                          <a class="dropdown-item" href="#" @click.prevent
                            ><Icon family="fal" name="sign-out" /> {{ $t("channels.leave-channel") }}</a
                          >
                          <template slot="popover">
                            <div class="dropdown-items">
                              <div class="dropdown-text">
                        ...
                        <v-popover v-if="ChannelRoles[channel.user_role].can('channel.manage') && user.id === channel.created_by">
                          <a class="dropdown-item" href="#" @click.prevent
                            ><Icon family="fal" name="trash-alt" /> {{ $t("channels.delete-channel") }}</a
                          >
                          <template slot="popover">
                            <div class="dropdown-items">
                              <div class="dropdown-text">
                  ...
                  <div class="close-meeting" v-if="channel.id == currentMeeting.channelId && currentMeeting.roomState !== 'closed'">
                    <button
                      v-tooltip="{
                        delay: 1000,
                        offset: -5,
          ...
          <a class="auto-archive-load-more" @click="onToggleArchived" v-if="autoArchivedAmount > 0">
...
import TeamHome from "@/components/Teams/TeamHome.vue";
...
export default {
  ...
  computed: {
    ...
    currentTeam() {
      return this.teamId ? this.$store.getters["teams/getTeamById"](this.teamId) : null;
    },
    userPreferences() {
    ...
    isDashboardActionsOn() {
      return (
        (this.channels.length === 0 && this.$store.state.loading["channel.list"]) ||
    ...
    channelList() {
      if (!this.showArchived) {
        return _.filter(this.channels, (o, i) => !o.is_autoarchived || o.unread_count != 0 || i < 11);
      } else {
