...
export default {
  ...
  methods: {
    ...
    async endDrag(event) {
      ...
      if (
        ...
        const channelActions =
          this.$store.getters['actions/getActionsByChannel'](channel_id);
        let newIndex = 0;
        const fromRoot =
          event.srcElement.classList.contains('nested-task-root');
        ...
        //to nested
          ...
          const under = to.id;
          const underAction = channelActions.find(
            (action) => action.id == under,
          );
    ...
    moveCallback(event) {
      ...
      const movingToNestedTask =
        !event.to.classList.contains('nested-task-root');
