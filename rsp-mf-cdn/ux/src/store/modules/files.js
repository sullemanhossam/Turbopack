...
import { transactionStatusPriority } from '../../constants/transactionStatusPriority';
import { priorityQueue, queue } from 'async-es';

// import { parseMessages, parseMessageContent, parseReactions, relDiff, asyncForEach, parseFile, parseMessageAttachments } from '../../utils';
...
export default {
  ...
  mutations: {
    ...
    add: (state, payload) => {
      ...
      //add in media
      if (payload.media) {
        const [beginOf, pathType, channelDir, ...finalPath] =
  ...
  actions: {
    ...
    onFileCopied(context, payload) {
      let { channel_id, fs_name, fs_path, user_id, media, previous_path } =
        payload;
